...
> 技术栈：Spring Boot 3.5.16 / Java 21 / LangChain4j 1.20.0 / MySQL / PostgreSQL(pgvector) / Redis / Redisson / Vue 3 / Element Plus
...
| AI 编排 | LangChain4j 1.20.0（AiServices、Tool calling、结构化输出、流式） |
...
**没有统一的 `MaskUtil` 工具类**，两处各自实现、规则也不同：

| 位置 | 规则 | 例 |
|---|---|---|
| `CustomerDto.maskIdCard`（客户管理接口） | 保留**前 6 位 + 后 4 位**，中间补 8 个 `*`；长度 ≤ 10 时只保留首位 + `****` | `110101199003078531` → `110101********8531` |
| `CustomerAssistantSnapshotFactory.maskName`（AI 小助快照） | 保留**首位** + `***`；空值返回「未知主体」 | `张伟` → `张***` |

工具轨迹不落参数明文，只落 digest。客户管理接口返回的字段是 `idCardMasked`——
**不含证件号明文**；但**姓名是原样返回的**（record 里就是 `name`），别说成"姓名也脱敏了"。

> ⚠️ 2026-09-19 更正：本节原写「脱敏（`MaskUtil`）」与「`maskName`（张伟→张*）、
> `maskIdCard`（110************56）」——**类名不存在，两个格式的保留位数与星号数也都不对**。
> 上表按代码改正，`maskIdCard` 那行是实跑出来的。
