...
// el-table 的 scoped slot 把 row 声明为 DefaultRow；模板里把它交给强类型函数
// 或用来索引强类型 Record 时，必须显式断言。
import type { AmlAlert, EnhancedDueDiligenceRequest } from '../api/client'
        ...
        >
          ...
          <div class="candidate-main">
            ...
            <div v-if="candidate.reviewDecision" class="candidate-review">
              <strong>人工核验：</strong>
              {{
                candidate.reviewDecision === 'CONFIRM'
                  ? '确认命中'
                  : candidate.reviewDecision === 'DISMISS'
                    ? '已排除'
                    : '等待补充材料'
              }}
    ...
    <div class="card">
      ...
      <el-descriptions :column="3" border size="small">
        ...
        <el-descriptions-item label="人工处置">{{
          caseItem.reviewDisposition
            ? (reviewDispositionText[caseItem.reviewDisposition] ?? caseItem.reviewDisposition)
            : '待处置'
        }}</el-descriptions-item>
      ...
      <el-descriptions v-if="caseOperations" :column="3" border size="small" class="operations-detail">
        ...
        <el-descriptions-item label="当前责任阶段">{{
          operationsPhaseText[caseOperations.phase]
        }}</el-descriptions-item>
        <el-descriptions-item label="责任队列">{{
          caseOperations.assignedTo || caseOperations.assignedUnit
        }}</el-descriptions-item>
        <el-descriptions-item label="阶段截止时间">
          <span :class="{ overdue: caseOperations.overdue }">{{
            caseOperations.dueAt ? fmtDateTime(caseOperations.dueAt) : '-'
          }}</span>
        </el-descriptions-item>
        <el-descriptions-item label="SLA 状态">
          <strong :class="caseOperations.overdue ? 'overdue' : 'ops-ok'">
            {{
              caseOperations.overdue
                ? `已逾期 ${Math.abs(caseOperations.minutesRemaining)} 分钟`
                : `剩余
      ...
      <div v-else-if="transactionWindows" class="window-grid">
        <div v-for="window in transactionWindows.windows" :key="window.days" class="window-card">
          ...
          <div class="window-metrics">
            <span
              >交易 <b>{{ window.transactionCount }}</b> 笔</span
            >
        ...
        <el-table-column v-if="canEditInvestigation || canSplitOnCase" label="操作" width="210" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              ...
              plain
              :loading="investigationActionId === row.id"
              @click="splitCaseAlert(row as AmlAlert)"
              >拆分</el-button
            >
          </template>
        </el-table-column>
        ...
        <div v-for="hypothesis in investigation.hypotheses" :key="hypothesis.id" class="hypothesis-card">
          ...
          <div v-else-if="canEditInvestigation" class="hypothesis-actions">
            ...
            <el-button
              ...
              plain
              :loading="investigationActionId === hypothesis.id"
              @click="decideInvestigationHypothesis(hypothesis, hypothesis.status as 'CONFIRMED' | 'REJECTED')"
              >重申判断</el-button
            >
            <el-button
              size="small"
    ...
    <div v-if="reviewHistory.length" class="card">
      ...
      <el-table :data="reviewHistory" stripe>
        ...
        <el-table-column label="原因" min-width="190">
          <template #default="{ row }">{{
            row.reasonCode ? (reviewReasonText[row.reasonCode] ?? row.reasonCode) : '-'
          }}</template>
        </el-table-column>
    ...
    <div v-if="eddRequests.length || eddHistoryLoadError" class="card">
      ...
      <el-table v-else :data="eddRequests" stripe>
        <el-table-column label="轮次" width="72">
          <template #default="{ row }"
            ><span class="mono-num">#{{ row.roundNo }}</span></template
          >
        </el-table-column>
        ...
        <el-table-column label="需补材料" min-width="220">
          <template #default="{ row }">{{ formatEddRequiredItems(row as EnhancedDueDiligenceRequest) }}</template>
        </el-table-column>
        ...
        <el-table-column v-if="canSubmitEdd" label="操作" width="110" fixed="right">
          <template #default="{ row }">
            <el-button
              ...
              size="small"
              type="primary"
              @click="openEddResponse(row as EnhancedDueDiligenceRequest)"
              >提交材料</el-button
            >
    ...
    <div v-if="suspiciousReport || reportWorkflowLoadError" class="card">
      ...
      <el-descriptions v-else-if="suspiciousReport" :column="2" border>
        <el-descriptions-item label="报告状态">{{
          reportStatusText[suspiciousReport.status] ?? suspiciousReport.status
        }}</el-descriptions-item>
      ...
      <div v-if="suspiciousReport?.status === 'SUBMITTED' && canReviewSanctions" class="report-actions">
        <el-button type="warning" plain :loading="returningReport" @click="returnReportForCorrection"
          >退回补正</el-button
        >
      </div>
    </div>
      ...
      <div class="report">
        <div v-if="caseItem?.reportSource || caseItem?.snapshotId" class="report-row">
          ...
          <div class="trace-line">
            ...
            <span v-if="caseItem?.snapshotId" class="mono-num trace-id"
              >快照 {{ snapPrefix(caseItem.snapshotId) }}</span
            >
          </div>
        </div>
