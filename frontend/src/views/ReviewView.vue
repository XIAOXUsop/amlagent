...
import { useReviewViewController } from './ReviewView.controller'

// el-table 的 scoped slot 把 row 声明为 DefaultRow；模板里把它交给强类型函数
// 或用来索引强类型 Record 时，必须显式断言。
import type { CaseItem, SuspiciousTransactionReport } from '../api/client'
defineOptions({ name: 'ReviewView' })

// 模板里的「查看案件 / 查看详情 / 打开案件调查链」都靠这个事件把工单号交回路由层。
// 模板抽成外置 .html 的那一版把它丢了——因为外置模板不参与类型检查，
// 少了 emit 也不会报错，只有真去点那几个按钮才会炸。
const emit = defineEmits<{ 'open-case': [id: number] }>()
    ...
    <div class="card">
      ...
      <el-table v-else :data="pendingReports" stripe>
        ...
        <el-table-column label="状态" width="130">
          <template #default="{ row }"
            >{{ row.status === 'RETURNED_FOR_CORRECTION' ? '退回补正' : '待报送' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="190">
          <template #default="{ row }">
            ...
            <el-button
              ...
              @click="markReportSubmitted(row as SuspiciousTransactionReport)"
              >登记报送</el-button
            >
          </template>
        </el-table-column>
    ...
    <div class="card">
      ...
      <el-table v-loading="loading" :data="pending" stripe>
        <el-table-column label="工单号" width="88">
          <template #default="{ row }"
            ><span class="mono-num case-id">#{{ row.id }}</span></template
          >
        </el-table-column>
...
<style scoped src="./styles/ReviewView.css"></style>
