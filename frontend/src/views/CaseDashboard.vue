...
import { useCaseDashboardController } from './CaseDashboard.controller'

// el-table 的 scoped slot 把 row 声明为 DefaultRow；模板里把它交给强类型函数
// 或用来索引强类型 Record 时，必须显式断言。
import type { AmlAlert, CaseItem, CaseOperationsView, CasePriority, OperationPhase } from '../api/client'
defineOptions({ name: 'CaseDashboard' })

const emit = defineEmits<{ 'open-case': [caseId: number] }>()
const {
  alertExternalId,
  alertHitReason,
  alertInbox,
  ...
  total,
  triagingAlertId,
} = useCaseDashboardController(emit)
</script>
  ...
  <div class="dashboard">
    ...
    <div class="overview card">
      ...
      <div class="ov-grid">
        <div class="ov-cell">
          ...
          <div>
            <b class="mono-num">{{ overview.pending }}</b
            ><span>待处理</span>
          </div>
        </div>
        <div class="ov-cell">
          ...
          <div>
            <b class="mono-num">{{ overview.running }}</b
            ><span>执行中</span>
          </div>
        </div>
        <div class="ov-cell">
          ...
          <div>
            <b class="mono-num">{{ overview.hold }}</b
            ><span>转人工</span>
          </div>
        </div>
        <div class="ov-cell">
          ...
          <div>
            <b class="mono-num">{{ overview.reportPending }}</b
            ><span>待报送</span>
          </div>
        </div>
        <div class="ov-cell">
          ...
          <div>
            <b class="mono-num">{{ overview.done }}</b
            ><span>已完成</span>
          </div>
        </div>
      </div>
      ...
      <el-table v-else v-loading="operationsLoading" :data="operations" stripe>
        <el-table-column label="优先级" width="100">
          <template #default="{ row }">
            <el-tag :type="priorityTag(row.priority as CasePriority)" effect="dark">{{
              priorityText[row.priority as CasePriority]
            }}</el-tag>
        ...
        <el-table-column label="案件 / 客户" width="150">
          <template #default="{ row }"
            ><span class="mono-num">#{{ row.caseId }}</span
            ><br />{{ row.customerName }}（{{ row.customerId }}）</template
          >
        </el-table-column>
        ...
        <el-table-column label="责任队列" width="165">
          <template #default="{ row }"
            >{{ row.assignedTo || row.assignedUnit }}<br /><span class="muted">{{
              row.responsibleRole
            }}</span></template
          >
        </el-table-column>
        ...
        <el-table-column label="阶段时限" width="195">
          <template #default="{ row }">
            <span class="mono-num" :class="{ 'ops-overdue': row.overdue }">{{ fmtDateTime(row.dueAt) }}</span
            ><br />
            <strong :class="row.overdue ? 'ops-overdue' : 'ops-remaining'">{{
              remainingText(row as CaseOperationsView)
            }}</strong>
          </template>
        </el-table-column>
    ...
    <div v-if="['ANALYST', 'ADMIN'].includes(currentUser?.role ?? '')" class="card">
      ...
      <el-table v-else :data="alertInbox" stripe>
        ...
        <el-table-column label="发生时间" width="168">
          <template #default="{ row }"
            ><span class="mono-num time">{{ fmtDateTime(row.occurredAt) }}</span></template
          >
        </el-table-column>
        <el-table-column label="分诊" width="300" fixed="right">
          <template #default="{ row }">
            <el-button
              ...
              @click="createAlertCase(row as AmlAlert, false)"
              >新建案件</el-button
            >
            <el-button
              size="small"
              ...
              plain
              :loading="triagingAlertId === row.id"
              @click="createAlertCase(row as AmlAlert, true)"
              >建案并调查</el-button
            >
    ...
    <div v-if="['ANALYST', 'ADMIN'].includes(currentUser?.role ?? '')" class="card">
      ...
      <el-table v-else :data="eddTasks" stripe>
        <el-table-column label="案件" width="90">
          <template #default="{ row }"
            ><span class="mono-num">#{{ row.caseId }}</span></template
          >
        </el-table-column>
        ...
        <el-table-column label="所需材料" min-width="220">
          <template #default="{ row }">{{
            row.requiredItems.map((item: string) => eddRequiredItemText[item] ?? item).join('、')
          }}</template>
        </el-table-column>
    ...
    <div class="card">
      ...
      <el-table v-loading="listLoading" :data="cases" stripe style="width: 100%">
        ...
        <el-table-column label="客户" width="130">
          <template #default="{ row }"
            >{{ row.customerName }}<span class="cid">{{ row.customerId }}</span></template
          >
        </el-table-column>
        ...
        <el-table-column label="人工处置" width="104">
          <template #default="{ row }">
            <span v-if="row.reviewDisposition" class="src">{{
              dispositionText[row.reviewDisposition] ?? row.reviewDisposition
            }}</span>
            <span v-else class="rk-none">-</span>
          </template>
        </el-table-column>
...
<style scoped src="./styles/CaseDashboard.css"></style>
