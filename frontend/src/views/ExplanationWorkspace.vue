...
import {
  amendExplanationUnit,
  captureExplanationEvidence,
  disposeExplanationIssue,
  getExplanationWorkspace,
  getExplanationReviewBasis,
  saveExplanationDraft,
  submitExplanationUnit,
  proposeIssueDowngrade,
  fetchUnitNextActions,
  type ExplanationIssueView,
  type ExplanationUnitView,
  type ExplanationWorkspaceView,
...
function captureContext(): WorkspaceContext {
  return { caseId: props.caseId, version: contextVersion }
}

function isCurrentContext(context: WorkspaceContext): boolean {
  return context.version === contextVersion && context.caseId === props.caseId
}
...
async function reload(context = captureContext()) {
  const requestSequence = ++reloadSequence
  loading.value = true
  try {
    const result = await getExplanationWorkspace(context.caseId)
    if (!isCurrentContext(context) || requestSequence !== reloadSequence) return
    workspace.value = result
  } catch (error: unknown) {
    if (!isCurrentContext(context) || requestSequence !== reloadSequence) return
    ElMessage.error(apiErrorMessage(error, '工作区加载失败，请刷新后重试'))
  } finally {
    if (isCurrentContext(context) && requestSequence === reloadSequence) loading.value = false
  }
}
...
watch(
  ...
  () => {
    ...
    nextActions.value = []
    nextActionsUnitId.value = null
    reviewBasisToken.value = ''
    void reload()
  },
)
onUnmounted(() => {
...
function openDraft(unit: ExplanationUnitView) {
  ...
  ElMessageBox.prompt(
    ...
    .catch((error: unknown) => {
      if (isCurrentContext(context) && !isDialogCancellation(error)) {
        ElMessage.error('草稿编辑器打开失败')
      }
    })
}
...
async function saveDraft(unit: ExplanationUnitView) {
  ...
  try {
    const updated = await saveExplanationDraft(context.caseId, unit.unitId, {
    ...
    if (isExplanationRevisionConflict(error)) {
      ...
      ElMessageBox.confirm(
        ...
        .catch((dialogError: unknown) => {
          if (!isCurrentContext(context)) return
          if (!isDialogCancellation(dialogError)) {
            ElMessage.error('草稿冲突处理失败，请重新打开编辑器')
            return
          }
          if (fresh?.draftJson) {
      ...
      ElMessage.error(apiErrorMessage(error, '草稿保存失败，请刷新后重试'))
    }
  } finally {
    if (isCurrentContext(context) && actionUnitId.value === unit.unitId) actionUnitId.value = null
  }
}
...
async function submitUnit(unit: ExplanationUnitView) {
  ...
  actionUnitId.value = unit.unitId
  try {
    const result = await submitExplanationUnit(context.caseId, unit.unitId, {
      expectedDraftRevision: unit.draftRevision,
      reviewBasisToken: reviewBasisToken.value,
      idempotencyKey: `SUBMIT:${context.caseId}:${unit.unitId}:${Date.now()}`,
    })
    if (!isCurrentContext(context)) return
    ...
    if (isExplanationRevisionConflict(error)) {
      ...
      ElMessage.error(apiErrorMessage(error, '提交失败，请刷新后重试'))
    }
  } finally {
    if (isCurrentContext(context) && actionUnitId.value === unit.unitId) actionUnitId.value = null
  }
...
async function amend(unit: ExplanationUnitView) {
  const context = captureContext()
  const currentSubmissionId = unit.currentSubmissionId
  if (currentSubmissionId === null) {
    ElMessage.warning('当前单元没有可修订的已采用提交')
    return
  }
  actionUnitId.value = unit.unitId
  try {
    ...
    if (!isCurrentContext(context)) return
    await amendExplanationUnit(context.caseId, unit.unitId, {
      currentSubmissionId,
    })
    if (!isCurrentContext(context)) return
    ...
    if (!isDialogCancellation(error)) {
      ElMessage.error(apiErrorMessage(error, '修订失败，请刷新后重试'))
    }
  } finally {
    if (isCurrentContext(context) && actionUnitId.value === unit.unitId) actionUnitId.value = null
  }
...
async function disposeIssue(issue: ExplanationIssueView, disposition: IssueDisposition) {
  ...
  try {
    ...
    await disposeExplanationIssue(context.caseId, issue.issueId, {
    ...
    if (!isCurrentContext(context)) return
    if (!isDialogCancellation(error)) {
      ElMessage.error(apiErrorMessage(error, '处置失败，请刷新后重试'))
    }
  }
}
...
async function proposeDowngrade(issue: ExplanationIssueView) {
  ...
  try {
    ...
    if (!isCurrentContext(context)) return
    const [reason, evidenceReference] = value.split('|').map((part) => part?.trim() ?? '')
    await proposeIssueDowngrade(context.caseId, issue.issueId, {
      expectedRevision: issue.revision,
    ...
    if (!isCurrentContext(context)) return
    if (!isDialogCancellation(error)) {
      ElMessage.error(apiErrorMessage(error, '降级提案失败'))
    }
  }
}
...
async function loadNextActions(unit: ExplanationUnitView) {
  const context = captureContext()
  const targetCaseId = context.caseId
  const targetUnitId = unit.unitId
  nextActionsUnitId.value = targetUnitId
  try {
    const actions = await fetchUnitNextActions(targetCaseId, targetUnitId)
    if (!isCurrentContext(context) || nextActionsUnitId.value !== targetUnitId) return
    nextActions.value = actions
  } catch (error: unknown) {
    if (!isCurrentContext(context) || nextActionsUnitId.value !== targetUnitId) return
    ElMessage.error(apiErrorMessage(error, '核验建议获取失败'))
  }
}

function nextActionKey(action: ExplanationNextActionView): string {
  return [
    action.category,
    action.relatedClaimCode ?? '',
    action.relatedIssueKey ?? '',
    action.missingFact,
...
async function captureEvidence() {
  const context = captureContext()
  try {
    await captureExplanationEvidence(context.caseId, {
    ...
    if (!isCurrentContext(context)) return
    ElMessage.error(apiErrorMessage(error, '材料登记失败，请刷新后重试'))
  }
}
...
async function loadReviewBasis() {
  const context = captureContext()
  try {
    const basis = await getExplanationReviewBasis(context.caseId)
    if (!isCurrentContext(context)) return
    ...
    if (!isCurrentContext(context)) return
    ElMessage.error(apiErrorMessage(error, '复核依据获取失败'))
  }
}
  ...
  <div class="card">
    ...
    <template v-else-if="workspace">
      <p class="hint">
        ...
        {{ explainCounts.suspicious }} 条有疑点、{{ explainCounts.unresolved }} 条未决 （混合结论允许并存；假设汇总为{{
          explainCounts.suspicious > 0
            ? '存在支持怀疑的发现'
            : explainCounts.unresolved > 0
              ? '存在未决单元'
              : '全部解释成立'
        }}）
      </p>
      <el-alert v-if="workspace.generalBlockers.length" type="warning" :closable="false" show-icon title="当前阻断项">
      ...
      <div class="basis-bar">
        ...
        <el-tag :type="workspace.canExclude ? 'success' : 'info'" size="small" effect="plain"
          >可排除 {{ workspace.canExclude ? '是' : '否' }}</el-tag
        >
        <el-tag :type="workspace.canConfirm ? 'danger' : 'info'" size="small" effect="plain"
          >可送确认可疑 {{ workspace.canConfirm ? '是' : '否' }}</el-tag
        >
      </div>
      ...
      <div v-for="unit in workspace.units" :key="unit.unitId" class="unit-card">
        ...
        <div class="unit-actions">
          ...
          <el-button size="small" type="primary" :loading="actionUnitId === unit.unitId" @click="saveDraft(unit)"
            >保存草稿</el-button
          >
          <el-button size="small" type="success" :loading="actionUnitId === unit.unitId" @click="submitUnit(unit)"
            >提交单元结论</el-button
          >
          <el-button
            v-if="unit.hasCurrentSubmission"
            size="small"
            type="warning"
            plain
            :loading="actionUnitId === unit.unitId"
            @click="amend(unit)"
            >修订</el-button
          >
        ...
        <div v-if="nextActionsUnitId === unit.unitId && nextActions.length" class="next-actions">
          ...
          <div v-for="action in nextActions" :key="nextActionKey(action)" class="next-action-row">
      ...
      <div v-for="issue in workspace.issues" :key="issue.issueId" class="issue-row">
        <el-tag
          :type="
            issue.severity === 'INTEGRITY_BLOCKER'
              ? 'danger'
              : issue.severity === 'DECISION_CRITICAL'
                ? 'warning'
                : 'info'
          "
          size="small"
          effect="plain"
          >{{ severityText[issue.severity] }}</el-tag
        >
        ...
        <template v-if="issue.disposition === 'OPEN'">
          ...
          <el-button size="small" type="warning" plain @click="disposeIssue(issue, 'DISCLOSED_UNRESOLVED')"
            >披露未解决</el-button
          >
          <el-button
            v-if="issue.severity !== 'INTEGRITY_BLOCKER'"
            size="small"
            @click="disposeIssue(issue, 'NOT_RELEVANT_WITH_REASON')"
            >说明不相关</el-button
          >
          <el-button
            v-if="issue.severity !== 'INTEGRITY_BLOCKER'"
      ...
      <div class="capture-bar">
        <el-select v-model="captureForm.sourceSystem" aria-label="材料来源系统" size="small" style="width: 180px">
        ...
        <el-input
          v-model="captureForm.sourceReference"
          aria-label="材料来源引用"
          size="small"
          placeholder="不透明来源引用（如 TXN-DOC-001）"
          style="width: 240px"
        />
      ...
      <div v-for="artifact in workspace.artifacts" :key="artifact.artifactVersionId" class="artifact-row muted">
        {{ artifact.artifactKey }} v{{ artifact.version }} · {{ artifact.sourceSystem }} · 完整性
        {{ artifact.integrityStatus }} · 可用性 {{ artifact.availability }}
      </div>
    </template>
  </div>
</template>
