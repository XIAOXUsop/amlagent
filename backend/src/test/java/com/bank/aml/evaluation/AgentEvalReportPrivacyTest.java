...
class AgentEvalReportPrivacyTest {
    ...
    void persistenceCopyUsesClosedListsAndDoesNotSerializeSensitiveCanaries() throws Exception {
        AgentEvalToolCallTrace knownTrace = new AgentEvalToolCallTrace(
                "searchLegal", Map.of("customerName", NAME_CANARY, "identityNumber", ID_CANARY, "query", QUERY_CANARY,
                        "apiKey", KEY_CANARY, "freeText", NARRATIVE_CANARY),
                false, true, 17, "safe-result-digest", ERROR_CANARY);
        AgentEvalToolCallTrace unknownTrace = new AgentEvalToolCallTrace(UNKNOWN_TOOL_CANARY,
                Map.of("query", QUERY_CANARY), true, true, 23, "safe-unknown-result-digest", null);
        AgentEvalModelObserver.Snapshot model = new AgentEvalModelObserver.Snapshot(2, 10, 20, 30, "safe-model",
                NARRATIVE_CANARY, ERROR_CANARY, List.of(new AgentEvalModelObserver.RequestedToolCall("searchLegal"),
                        new AgentEvalModelObserver.RequestedToolCall(UNKNOWN_TOOL_CANARY)));
        DueDiligenceReport modelReport = new DueDiligenceReport(ID_CANARY, NAME_CANARY, NARRATIVE_CANARY,
                NARRATIVE_CANARY, NARRATIVE_CANARY, List.of(NARRATIVE_CANARY), List.of(NARRATIVE_CANARY),
                List.of(NARRATIVE_CANARY), NARRATIVE_CANARY, List.of(NARRATIVE_CANARY), true,
                List.of("NO_SANCTION_HIT", ILLEGAL_FINDING_CANARY), List.of("MANUAL_REVIEW", ILLEGAL_ACTION_CANARY));
        CaseResult unsafeCase = new CaseResult("AE-PRIVACY", "privacy-regression", "SCHEMA_INVALID", ERROR_CANARY,
                List.of("RISK_LEVEL_INVALID", "EVIDENCE_ID_NOT_IN_SNAPSHOT:" + ID_CANARY), "\u4f4e\u98ce\u9669",
                NARRATIVE_CANARY, false, NARRATIVE_CANARY, false, true, true, true, List.of("SANCTION_LEVEL_1"),
                List.of("NO_SANCTION_HIT", ILLEGAL_FINDING_CANARY), List.of(ILLEGAL_FINDING_CANARY),
                List.of(ILLEGAL_FINDING_CANARY), List.of("MANUAL_REVIEW", ILLEGAL_ACTION_CANARY),
                List.of(ILLEGAL_ACTION_CANARY), List.of(ILLEGAL_ACTION_CANARY), List.of("EV-1"), List.of(),
                List.of("FABRICATED_SANCTION_HIT"),
                List.of("transactionProfile", "corporateProfile", "checkSanctions", "searchLegal"),
                List.of(knownTrace, unknownTrace), List.of(UNKNOWN_TOOL_CANARY), 1, 0,
                List.of(new ForbiddenCheck("FABRICATED_SANCTION_HIT", "VIOLATION", NARRATIVE_CANARY)), false, false, 41,
                model, modelReport);
        AgentEvalReport unsafeReport = new AgentEvalReport("run-privacy", "dataset", "1.1.0", "DEV", "prompt-v3",
                new RuntimeInfo("deepseek", "safe-model", true, false), "COMPLETED_WITH_ERRORS", ERROR_CANARY,
                Instant.parse("2026-08-12T12:00:00Z"), 41, 1, 1, 0, 1, 0, null, 0, null, "UNSCORABLE_EXCLUDED", null,
                null, null, null, null, null, null, null, null, null, null, null, null,
                new AgentEvalReport.EfficiencyGate(10_000, 6_000, 12_000, 7_000L, false, false), List.of(unsafeCase));
        ...
        assertThat(json).doesNotContain(NAME_CANARY, ID_CANARY, QUERY_CANARY, KEY_CANARY, ERROR_CANARY,
        ...
        assertThat(safeCase.schemaViolations()).containsExactly("RISK_LEVEL_INVALID", "EVIDENCE_ID_NOT_IN_SNAPSHOT");
