...
class ForbiddenClaimDetectorRegistryTest {
    ...
    void catchesFabricatedSanctionHitOnNormalCustomer() {
        ...
        var report = report(evalCase, "低风险", false, List.of("NORMAL_TRANSACTION_PATTERN", "SANCTION_LEVEL_1_MATCH"),
                List.of("MAINTAIN_STANDARD_MONITORING"));

        assertThat(registry.evaluate(evalCase, report)).anySatisfy(check -> {
    ...
    void unsupportedFreeTextClaimIsExplicitlyUnscorable() {
        ...
        var report = report(evalCase, "中风险", true, evalCase.expected().requiredFindingCodes(),
    ...
    private DueDiligenceReport report(AgentEvalDataset.AgentEvalCase evalCase, String risk, boolean manual,
            List<String> findings, List<String> actions) {
        return new DueDiligenceReport(evalCase.input().customerId(), evalCase.input().customerName(), risk, "tx",
                "corp", new ArrayList<>(), List.of("law"), List.of("risk"), "conclusion", List.of("evidence"), manual,
                findings, actions);
    }
