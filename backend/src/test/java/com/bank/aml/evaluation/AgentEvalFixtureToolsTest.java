...
class AgentEvalFixtureToolsTest {
    ...
    void exposesExactlyTheFourStableToolNames() {
        List<String> toolNames = List.of(AgentEvalFixtureTools.class.getDeclaredMethods())
            .stream()
        ...
        assertThat(toolNames).containsExactly("checkSanctions", "corporateProfile", "searchLegal",
                "transactionProfile");
    }
    ...
    void rejectsMismatchedArgumentsWithoutReturningAnyFixtureData() {
        ...
        List<String> invalidResults = List.of(caseOne.transactionProfile("E2002"), caseOne.corporateProfile("e1001"),
                caseOne.checkSanctions("E2002"), caseOne.checkSanctions(null));

        assertThat(invalidResults).containsOnly(AgentEvalFixtureTools.ARGUMENT_VALIDATION_FAILED);
        assertThat(invalidResults)
            .allSatisfy(result -> assertThat(result).doesNotContain("E1001-SECRET", "E2002-SECRET"));
        assertThat(caseOne.searchLegal("  ")).isEqualTo(AgentEvalFixtureTools.LEGAL_QUERY_VALIDATION_FAILED);
        assertThat(caseOne.searchLegal("讲一个笑话")).isEqualTo(AgentEvalFixtureTools.LEGAL_QUERY_VALIDATION_FAILED);
    ...
    void legalSearchUsesVisibleKeywordContractAndRejectsParaphrases() {
        ...
        assertThat(tools.traces()).extracting(AgentEvalToolCallTrace::argumentValid).containsExactly(true, false);
    }
    ...
    void capturesConcurrentCallsWithoutDroppingTraces() throws Exception {
        ...
        try {
            List<Future<String>> futures = IntStream.range(0, 64)
    ...
    private static AgentEvalDataset.AgentEvalCase evalCase(String customerId, String customerName,
            String identityNumber) {
        AgentEvalDataset.AgentInput input = new AgentEvalDataset.AgentInput(customerId, customerName, identityNumber,
                "INDIVIDUAL", "2026-08-12", "alert", "case");
        AgentEvalDataset.RiskFacts riskFacts = new AgentEvalDataset.RiskFacts(0, 0, 0, true, true, 0, 0, false, 0);
        AgentEvalDataset.ToolFixture fixture = new AgentEvalDataset.ToolFixture("TX-" + customerId + "-SECRET",
                "CORP-" + customerId + "-SECRET", "SANCTION-" + customerId + "-SECRET", "legal query",
                List.of("customer due diligence", "AML"), "LEGAL-" + customerId + "-SECRET", riskFacts);
        AgentEvalDataset.ExpectedOutcome expected = new AgentEvalDataset.ExpectedOutcome("LOW", false, List.of(),
                List.of(), List.of(), List.of(), List.of(), List.of(), List.of(), List.of());
        AgentEvalDataset.Annotation annotation = new AgentEvalDataset.Annotation("rationale", List.of(),
                "PENDING_DOMAIN_REVIEW", "review");
        return new AgentEvalDataset.AgentEvalCase("CASE-" + customerId, "DEV", "SCENARIO", "EASY", input, fixture,
                expected, annotation);
    }
