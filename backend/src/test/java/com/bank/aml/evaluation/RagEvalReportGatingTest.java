...
/**
 * FR-04（v4 计划 §4.4 / RF-29）：A/B 评测必须记录实际执行管线；
 * 声称包含重排的管线一次 rerank 都没发生 → 环境失败，不冒充"四路比较"。
...
class RagEvalReportGatingTest {
    ...
    void rerankGatingFieldsExistWithCompatConstructor() {
        ...
        RagEvaluator.RagEvalReport legacy = new RagEvaluator.RagEvalReport(
                18, 100.0, 100.0, 95.6, 96.7, 100.0, 100.0,
                671.0, 300.0, 671.0, 700.0, 100.0, 120.0, 150.0,
                new RagEvaluator.SegmentedLatency(java.util.Map.of("rerank", 200.0)),
                "v1", "hash", "PENDING_DOMAIN_REVIEW", java.util.List.of(), "PRODUCTION");
    ...
    void rerankPipelineWithoutInvocationIsEnvironmentFailure() {
        ...
        RagEvaluator.RagEvalReport report = new RagEvaluator.RagEvalReport(
                18, 0.0, 0.0, 0.0, 0.0, 100.0, 100.0,
                10.0, 8.0, 10.0, 12.0, 9.0, 11.0, 13.0,
                new RagEvaluator.SegmentedLatency(java.util.Map.of()),
                "v1", "hash", "PENDING_DOMAIN_REVIEW", java.util.List.of(),
                RetrievalPipeline.HYBRID_RERANK.name(), 0, true);
    ...
    void rerankPipelineWithInvocationIsNotEnvironmentFailure() {
        RagEvaluator.RagEvalReport report = new RagEvaluator.RagEvalReport(
                ...
                new RagEvaluator.SegmentedLatency(java.util.Map.of("rerank", 200.0)),
                "v1", "hash", "PENDING_DOMAIN_REVIEW", java.util.List.of(),
                RetrievalPipeline.HYBRID_RERANK.name(), 18, false);
    ...
    void nonRerankPipelineIgnoresGating() {
        ...
        RagEvaluator.RagEvalReport report = new RagEvaluator.RagEvalReport(
                18, 93.3, 93.3, 81.1, 84.2, 100.0, 100.0,
                135.0, 100.0, 135.0, 140.0, 50.0, 60.0, 70.0,
                new RagEvaluator.SegmentedLatency(java.util.Map.of()),
                "v1", "hash", "PENDING_DOMAIN_REVIEW", java.util.List.of(),
                RetrievalPipeline.HYBRID.name(), 0, false);
        assertThat(report.environmentFailure()).isFalse();
    }
}
