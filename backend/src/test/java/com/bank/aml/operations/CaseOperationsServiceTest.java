...
class CaseOperationsServiceTest {
    ...
    private final CaseOperationsService service = new CaseOperationsService(cases, alerts, edd, executions, reports,
            clock);
    ...
    private final CaseOperationsService investigationAwareService = new CaseOperationsService(cases, alerts, edd,
            executions, reports, hypotheses, coverage, evidence, new InvestigationReadinessEvaluator(), null, clock);
    ...
    void sanctionAlertIsCriticalAndSlaStartsWhenAlertEnteredSystem() {
        ...
        assertThat(result.clockStartedAt()).isEqualTo(now.minusHours(5).toInstant(ZoneOffset.UTC));
        assertThat(result.dueAt()).isEqualTo(now.minusHours(1).toInstant(ZoneOffset.UTC));
    ...
    void openEddUsesExplicitDeadlineAndNamedAssigneeAtExactBoundary() {
        ...
        assertThat(result.dueAt()).isEqualTo(now.toInstant(ZoneOffset.UTC));
        assertThat(result.overdue()).isTrue();
        ...
        assertThat(result.calculatedAt()).isEqualTo(now.toInstant(ZoneOffset.UTC));
    }
    ...
    void returnedReportRestartsReportingClockAndHasCriticalPriorityFloor() {
        ...
        assertThat(result.clockStartedAt()).isEqualTo(now.minusHours(1).toInstant(ZoneOffset.UTC));
        assertThat(result.dueAt()).isEqualTo(now.plusHours(3).toInstant(ZoneOffset.UTC));
    }
    ...
    void duplicateAlertsFromSameScenarioDoNotInflateScenarioWeight() {
        ...
        assertThat(result.priorityReasons()).containsExactly("交易与客户画像不匹配", "同一客户聚合 2 条有效预警");
    }
    ...
    void queuesAreSeparatedByRoleAndEddAssignee() {
        ...
        when(cases.findByStatusInOrderByCreatedAtAsc(any()))
        ...
        assertThat(service.queue("analyst-a", "ANALYST", false, null, null)).extracting(CaseOperationsView::caseId)
            .containsExactlyInAnyOrder(1L, 2L);
        assertThat(service.queue("analyst-b", "ANALYST", false, null, null)).extracting(CaseOperationsView::caseId)
            .containsExactly(1L);
        assertThat(service.queue("reviewer", "REVIEWER", false, null, null)).extracting(CaseOperationsView::caseId)
            .containsExactlyInAnyOrder(3L, 4L);
        assertThat(service.queue("admin", "ADMIN", false, null, null)).extracting(CaseOperationsView::caseId)
    ...
    void holdWithIncompleteInvestigationBelongsToAnalystInvestigationQueue() {
        ...
        when(cases.findByStatusInOrderByCreatedAtAsc(any())).thenReturn(List.of(caseEntity));
        ...
        when(hypotheses.findByCaseIdInOrderByIdAsc(any())).thenReturn(List.of(open));
        AlertInvestigationCoverage pending = mock(AlertInvestigationCoverage.class);
        when(pending.getAlertId()).thenReturn(11L);
        when(pending.getConclusion()).thenReturn(AlertCoverageConclusion.PENDING);
        when(coverage.findByCaseIdOrderByAlertIdAsc(8L)).thenReturn(List.of(pending));
        when(coverage.findByCaseIdInOrderByAlertIdAsc(any())).thenReturn(List.of(pending));
        when(evidence.findByCaseIdOrderByCreatedAtAsc(8L)).thenReturn(List.of());
        when(evidence.findByCaseIdIn(any())).thenReturn(List.of());
        ...
        assertThat(investigationAwareService.queue("analyst-a", "ANALYST", false, null, null))
            .extracting(CaseOperationsView::caseId)
            .contains(8L);
    ...
    void holdWithReadyInvestigationGoesToReviewerAndReviewStartUsesPersistedEvents() {
        ...
        InvestigationEvidenceLink support = evidence(21L, InvestigationEvidenceType.TRANSACTION,
                EvidenceStance.SUPPORTS);
        when(hypotheses.findByCaseIdOrderByIdAsc(9L)).thenReturn(List.of(confirmed));
        when(hypotheses.findByCaseIdInOrderByIdAsc(any())).thenReturn(List.of(confirmed));
        when(coverage.findByCaseIdOrderByAlertIdAsc(9L)).thenReturn(List.of(suspicious));
        when(coverage.findByCaseIdInOrderByAlertIdAsc(any())).thenReturn(List.of(suspicious));
        when(evidence.findByCaseIdOrderByCreatedAtAsc(9L)).thenReturn(List.of(support));
        when(evidence.findByCaseIdIn(any())).thenReturn(List.of(support));
        ...
        assertThat(first.clockStartedAt()).isEqualTo(now.minusHours(1).toInstant(ZoneOffset.UTC));
    ...
    private AlertInvestigationCoverage suspiciousCoverage(LocalDateTime updatedAt) {
        ...
        when(item.getConclusion()).thenReturn(AlertCoverageConclusion.SUSPICIOUS);
    ...
    private InvestigationEvidenceLink evidence(Long hypothesisId, InvestigationEvidenceType type,
        ...
        InvestigationEvidenceLink link = mock(InvestigationEvidenceLink.class);
