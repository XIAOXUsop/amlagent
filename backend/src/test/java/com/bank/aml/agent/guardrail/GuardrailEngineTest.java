...
class GuardrailEngineTest {
    ...
    void setUp() {
        ...
        engine = new GuardrailEngine(new RiskFactAssembler(dataSource),
                new RiskRuleEngine(repository, TestClocks.FIXED));
    }
    ...
    void level1SanctionOnlyTriggersLevel1RuleAndRequiresManualReview() {
        ...
        assertThat(result.decision().triggeredRules()).extracting(RiskRuleEngine.TriggeredRule::ruleCode)
    ...
    void evaluationContextCanRunWithoutAccessingMockDataSource() {
        ...
        RiskRuleRepository repository = mock(RiskRuleRepository.class);
        when(repository.findByEnabledTrueOrderByPriorityAsc()).thenReturn(seedRules());
        GuardrailEngine evaluationEngine = new GuardrailEngine(new RiskFactAssembler(unusedDataSource),
                new RiskRuleEngine(repository, TestClocks.FIXED));
    ...
    private RiskContext context(int maxSeverity, boolean sanctionHit, double crossRatio, double nightRatio,
        ...
        return new RiskContext(maxSeverity, sanctionHit, crossRatio, nightRatio, 0, dataComplete, riskExplained,
                patternSeverity, uboRiskSeverity, "占位值", 0);
    }
    ...
    private DueDiligenceReport report(String riskLevel, boolean manualReviewRequired) {
        return new DueDiligenceReport("C001", "张伟", riskLevel, "", "", List.of(), List.of(), List.of(riskLevel), "结论",
                List.of(), manualReviewRequired, List.of(), List.of());
    }

    private List<RiskRule> seedRules() {
        return List.of(rule("SANCTION_LEVEL_1", "sanction.maxSeverity == 1", "高风险", "MANUAL_REVIEW", 10),
                ...
                rule("CROSS_BORDER_MODERATE", "transaction.crossRatio >= 10 && transaction.riskExplained == false",
                        "中风险", "AUTO_DONE", 90),
                rule("NIGHT_ACTIVITY_MODERATE", "transaction.nightRatio >= 20 && transaction.riskExplained == false",
                        "中风险", "AUTO_DONE", 100));
    }
