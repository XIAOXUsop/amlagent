...
class LegalEvidenceSupportValidatorTest {
    ...
    void authorityReportRequiresARealCitedSupportingArticle() {
        LegalDoc law = new LegalDoc("LEGAL-REPORT-1", "金融机构大额交易和可疑交易报告管理办法", "文号", "第十条", "金融机构发现可疑交易后，应当及时提交可疑交易报告。");
    ...
    void authorityReportAlsoAcceptsCitedSanctionReportingDuty() {
        ...
        assertThat(validator.validate(snapshot, report(List.of("依据 LEGAL-SANCTION-1"), List.of("LEGAL-SANCTION-1"))))
            .isEmpty();
    }
    ...
    void negatedFreezeClauseDoesNotSupportFreezeAction() {
        ...
        InvestigationSnapshot snapshot = mock(InvestigationSnapshot.class);
        when(snapshot.legalEvidence()).thenReturn(List.of(law));
        DueDiligenceReport report = new DueDiligenceReport("C1", "客户", "高风险", "交易", "主体", List.of(),
                List.of("依据 LEGAL-FREEZE-1"), List.of("风险"), "冻结", List.of("LEGAL-FREEZE-1"), true,
                List.of("SANCTION_LEVEL_1_MATCH"), List.of("FREEZE_ASSETS", "MANUAL_REVIEW"));

        // “不得冻结”条款不能被当作支持冻结的依据
        assertThat(validator.validate(snapshot, report)).containsExactly("FREEZE_ASSETS_LEGAL_SUPPORT_MISSING");
    }
    ...
    void freezeActionIsSupportedByConstructiveFreezeClause() {
        ...
        when(snapshot.legalEvidence()).thenReturn(List.of(law));
        DueDiligenceReport report = new DueDiligenceReport("C1", "客户", "高风险", "交易", "主体", List.of(),
                List.of("依据 LEGAL-FREEZE-2"), List.of("风险"), "冻结", List.of("LEGAL-FREEZE-2"), true,
                List.of("SANCTION_LEVEL_1_MATCH"), List.of("FREEZE_ASSETS", "MANUAL_REVIEW"));
    ...
    private DueDiligenceReport report(List<String> basis, List<String> evidence) {
        return new DueDiligenceReport("C1", "客户", "高风险", "交易", "主体", List.of(), basis, List.of("风险"), "转主管机关", evidence,
                true, List.of("RISK_ASSESSMENT_UNCERTAIN"), List.of("REPORT_TO_AUTHORITY", "MANUAL_REVIEW"));
    }
