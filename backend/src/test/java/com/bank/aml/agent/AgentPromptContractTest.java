...
class AgentPromptContractTest {
    ...
    void promptContainsEveryProductionVocabularyCode() throws Exception {
        ...
        assertThat(AgentReportVocabulary.FINDING_CODES).allSatisfy(code -> assertThat(prompt).contains(code));
        assertThat(AgentReportVocabulary.ACTION_CODES).allSatisfy(code -> assertThat(prompt).contains(code));
    }
    ...
    void promptRequiresExplicitEvidenceAndForbidsGenericActionPadding() throws Exception {
        ...
        assertThat(prompt).contains("事实排他")
    ...
    void promptContainsCriticalFactToDecisionMappings() throws Exception {
        ...
        assertThat(prompt).contains("必须完成以下闭环映射")
            .contains("一级制裁身份精确命中")
            .contains("交易数据不可用")
