...
class TransactionWindowServiceTest {
    ...
    void buildsCustomerWindowsAtSourceAsOfAndExcludesFutureTransactions() {
        ...
        when(data.transactionsOf("C001"))
            .thenReturn(List.of(tx(localAsOf.minusDays(10).withHour(23), "100.00", "入账", "A", CountryRegion.CHINA),
    ...
    private TransactionRecord tx(LocalDateTime date, String amount, String direction, String counterparty,
            CountryRegion country) {
        return new TransactionRecord(date, new BigDecimal(amount), direction, counterparty, country, "网银", "转账",
                country.isCrossBorder() ? "USD" : "CNY");
    }
