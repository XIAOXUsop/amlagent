 ...
 * 原路不自动免责（RF-07）；买方权限需核实（RF-08/09）；无关账户权限未知阻断（RF-09/10）； 影响评估四类全覆盖、禁止"无影响"。
 */
class RefundAuthorityTest {
    ...
    void originalPathWithVerifiedReasonIsAdmissible() {
        var verdict = service.assess(RefundAuthorityService.RecipientAuthority.ORIGINAL_PAYER_VERIFIED,
    ...
    void buyerWithVerifiedAuthorityIsAdmissible() {
        var verdict = service.assess(RefundAuthorityService.RecipientAuthority.BUYER_VERIFIED,
                RefundAuthorityService.CommercialReason.VERIFIED);
        assertThat(verdict.admissible()).isTrue();
    ...
    void unresolvedAuthorityBlocksExplanation() {
        var verdict = service.assess(RefundAuthorityService.RecipientAuthority.UNRESOLVED,
                RefundAuthorityService.CommercialReason.VERIFIED);
        assertThat(verdict.admissible()).isFalse();
    ...
    void contradictedAuthorityBlocksAndKeepsContradiction() {
        var verdict = service.assess(RefundAuthorityService.RecipientAuthority.CONTRADICTED,
                RefundAuthorityService.CommercialReason.VERIFIED);
        assertThat(verdict.admissible()).isFalse();
    ...
    void branchSemanticsMatchFixedCase() {
        assertThat(RefundAuthorityService.branchSemantics()).containsKeys("ORIGINAL_PATH", "TO_BUYER", "TO_UNRELATED");
    }
