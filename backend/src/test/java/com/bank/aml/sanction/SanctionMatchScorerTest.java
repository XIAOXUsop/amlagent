...
class SanctionMatchScorerTest {
    ...
    private final CustomerProfile customer = new CustomerProfile("C001", "张伟", "110101198506123456", "自然人", "零售", "北京",
            "-");

    @Test
    void confirmsExactIdentityAndKeepsIdentityMasked() {
        var match = scorer.score(customer,
                new SanctionRecord("ZHANG WEI（张伟）", "110101198506123456", "OFAC SDN", "reason", 1));
    ...
    void dismissesCompanyWhoseNameOnlyContainsNaturalPersonName() {
        var match = scorer.score(customer,
                new SanctionRecord("ZHANGWEI TRADING CO., LTD.（张伟国际贸易有限公司）", "", "OFAC SDN", "reason", 1));
    ...
    void identityConflictOverridesSameName() {
        var match = scorer.score(customer, new SanctionRecord("张伟", "999999999999999999", "WATCHLIST", "reason", 2));
    ...
    void riskAssemblerFiltersLowConfidenceCompanySubstringFromGuardrailFacts() {
        ...
        assertThat(new RiskFactAssembler(source, scorer).searchSanctions(customer)).containsExactly(person);
    }
