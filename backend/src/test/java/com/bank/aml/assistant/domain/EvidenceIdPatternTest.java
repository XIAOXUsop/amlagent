 ...
 * 证据标识识别规则的测试。
 *
 * <p>存在意义：此前 {@code AssistantOutputGuard} 与 {@code AssistantEvidenceCitationAppender}
 * 各自定义正则且宽窄不一致，护栏只认 64 位小写十六进制，**扫不到**模型吐出的大写/非 64 位标识，
 * 使伪造引用可以穿过输出校验。本测试把统一后的行为钉死。
 */
class EvidenceIdPatternTest {
    ...
    void matchesEveryFormTheModelMayEmit() {
        ...
        assertThat(matches("TRANSACTION_AGGREGATE:"
                + "71b39e0804a17ef7cdc8508545810921059266fbc43fdb4446d90f10c235595b")).isTrue();
     ...
     * 回归证明：这条断言在修复前会**失败**。
     *
     * <p>伪造 ID 是大写且长度不为 64，落在旧护栏正则的扫描范围之外 →
     * {@code EVIDENCE_NOT_IN_SNAPSHOT} 不会触发 → 校验误判为通过。
     */
    @Test
