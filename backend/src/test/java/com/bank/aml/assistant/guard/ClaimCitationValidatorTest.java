...
class ClaimCitationValidatorTest {
    ...
    private final AssistantEvidence legal = new AssistantEvidence("LEGAL-FREEZE-1",
            AssistantEvidence.EvidenceType.AML_LEGAL, "涉及恐怖活动资产冻结管理办法", "金融机构应当立即对相关资产采取冻结措施，未按要求冻结可能面临处理。",
            "OFFICIAL");
    ...
    private CustomerAssistantSnapshot snapshot(AssistantEvidence... evidence) {
        return new CustomerAssistantSnapshot("s", "c", "r", Instant.EPOCH,
                ...
                new TransactionRiskView(2, new BigDecimal("10"), new BigDecimal("5"), 0, 0, 0, 0, List.of("CNY"),
                        List.of("中国大陆"), true),
                new OwnershipRiskView(0, 0, List.of()), new SanctionRiskView(false, 0, 0, List.of()), List.of(evidence),
    ...
    void legalClaimMustCiteASnapshotLegalEvidence() {
        assertThat(validator.validate(snapshot, List.of(claim("金融机构应当立即冻结相关资产", List.of(), List.of()))))
            .anyMatch(v -> v.startsWith("CLAIM_LEGAL_WITHOUT_EVIDENCE"));
        assertThat(validator.validate(snapshot, List.of(claim("金融机构应当立即冻结相关资产", List.of("LEGAL-INVENTED"), List.of()))))
            .anyMatch(v -> v.startsWith("CLAIM_EVIDENCE_NOT_IN_SNAPSHOT"));
        assertThat(validator.validate(snapshot, List.of(claim("金融机构应当履行报告义务", List.of("LEGAL-FREEZE-1"), List.of()))))
            .anyMatch(v -> v.startsWith("CLAIM_LEGAL_WITHOUT_SUPPORT_SPAN"));
    }

    @Test
    void validClaimPassesWithRealSpanAndNumber() {
        AssistantEvidence second = new AssistantEvidence("LEGAL-FREEZE-2", AssistantEvidence.EvidenceType.AML_LEGAL,
                "涉及恐怖活动资产冻结管理办法", "金融机构应当立即对相关资产采取冻结措施，并向主管机关报告，不得拖延。", "OFFICIAL");
        CustomerAssistantSnapshot snapshot = snapshot(legal, second);
        // 高风险冻结声明引用两条法律证据，模态与原文一致，支持片段为原文连续子串
        assertThat(validator.validate(snapshot, List.of(claim("金融机构应当立即对相关资产采取冻结措施，并向主管机关报告",
                List.of("LEGAL-FREEZE-1", "LEGAL-FREEZE-2"), List.of("金融机构应当立即对相关资产采取冻结措施")))))
            .isEmpty();
    }
    ...
    void unsupportedOrFragmentedSpanIsRejected() {
        AssistantEvidence second = new AssistantEvidence("LEGAL-FREEZE-2", AssistantEvidence.EvidenceType.AML_LEGAL,
                "涉及恐怖活动资产冻结管理办法", "金融机构应当立即对相关资产采取冻结措施，并向主管机关报告。", "OFFICIAL");
        CustomerAssistantSnapshot snapshot = snapshot(legal, second);
        // 片段为原文连续子串且足够长 → 通过；换置信合法 span
        assertThat(validator.validate(snapshot, List
            .of(claim("金融机构应当立即对相关资产采取冻结措施", List.of("LEGAL-FREEZE-1", "LEGAL-FREEZE-2"), List.of("应当立即对相关资产采取冻结")))))
            .isEmpty();
        assertThat(validator.validate(snapshot, List
            .of(claim("金融机构应当立即对相关资产采取冻结措施", List.of("LEGAL-FREEZE-1", "LEGAL-FREEZE-2"), List.of("完全不存在的原文片段")))))
    ...
    void modalityMismatchBetweenClaimAndEvidenceIsRejected() {
        ...
        assertThat(validator.validate(snapshot,
                List.of(claim("金融机构不得解除冻结措施", List.of("LEGAL-FREEZE-1"), List.of("金融机构应当立即对相关资产采取冻结措施")))))
    ...
    void highRiskFreezeClaimNeedsMultipleLegalSupports() {
        assertThat(validator.validate(snapshot,
                List.of(claim("金融机构应当立即冻结并报告制裁名单命中主体", List.of("LEGAL-FREEZE-1"), List.of("金融机构应当立即对相关资产采取冻结措施")))))
