...
class AssistantRunOrchestratorTest {
    ...
    void sensitiveInputIsRedactedAndRefusedWithoutSchedulingAgent() {
        ...
        AssistantRunOrchestrator orchestrator = new AssistantRunOrchestrator(conversations, state, input,
                new AssistantOutputGuard(sensitive), mock(CustomerAssistantSnapshotFactory.class),
                mock(AssistantSnapshotArchiveService.class), mock(AssistantHistoryLoader.class),
                mock(CustomerAssistantAgentFactory.class), mock(ConversationLeaseService.class), executor,
                mock(ScheduledExecutorService.class), events, properties);

        orchestrator.submitMessage("c", "admin", "m-1", "身份证110101199001011234");

        verify(conversations).acceptMessage("c", "admin", "m-1", "身份证[敏感信息已遮蔽]");
        verify(state).refuse("r", "请勿在对话中输入或索取完整身份证、账号、银行卡或密钥信息。", AssistantResultType.SENSITIVE_DATA_DENIED,
