...
class FinalReportStreamingServiceTest {
    ...
    void streamsOnlyFinalStructuredDecisionWithoutSecondModelCall() {
        ...
        assertThat(service.renderChunks(report)).containsExactly("最终风险评级：高风险。\n", "风险发现：SANCTION_LEVEL_1_MATCH。\n",
                "处置动作：MANUAL_REVIEW。\n", "该工单需要人工复核。");
    ...
    private DueDiligenceReport report() {
        return new DueDiligenceReport("C001", "[trusted]", "高风险", "summary", "conclusion-with-untrusted-free-text",
                List.of(), List.of(), List.of(), "actions", List.of(), true, List.of("SANCTION_LEVEL_1_MATCH"),
