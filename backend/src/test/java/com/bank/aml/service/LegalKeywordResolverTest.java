...
class LegalKeywordResolverTest {
    ...
    void sanctionFactsAddLegalTopicsEvenWhenAlertTextDoesNotMentionSanctions() {
        RiskContext riskFacts = new RiskContext(1, true, 0, 0, 0,
                true, false, 0, 0, null, 0);

        assertThat(resolver.resolve("大额频繁跨国转账、夜间集中交易", riskFacts))
                .contains("跨境", "夜间", "资产冻结", "停止金融服务", "冻结措施报告");
    }
    ...
    void frozenAlertsKeepPerAlertKeywordsAndScenarioTopics() {
        RiskContext riskFacts = new RiskContext(0, false, 0, 0, 0,
                true, true, 0, 0, "低风险", 1);
        var crossBorder = new com.bank.aml.domain.InvestigationAlertSnapshot(11L, "ALERT-A",
                "RULE-001", "CROSS_BORDER_ANOMALY", "客户连续发生夜间跨境转账",
                java.time.LocalDateTime.of(2026, 8, 1, 23, 0), 0);
        var structuring = new com.bank.aml.domain.InvestigationAlertSnapshot(12L, "ALERT-B",
    ...
    void scenarioCodeAloneContributesDeterministicTopic() {
        RiskContext riskFacts = new RiskContext(0, false, 0, 0, 0,
                true, true, 0, 0, "低风险", 1);
        var sanctions = new com.bank.aml.domain.InvestigationAlertSnapshot(13L, "ALERT-C",
                "RULE-003", "SANCTIONS_WATCHLIST", "RULE-003",
                java.time.LocalDateTime.of(2026, 8, 1, 12, 0), 0);
