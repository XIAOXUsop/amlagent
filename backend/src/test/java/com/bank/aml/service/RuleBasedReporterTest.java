...
class RuleBasedReporterTest {
    ...
    void safetyHoldIsStructuredAndCannotRetainNormalFinding() {
        InvestigationSnapshot snapshot = snapshot(new RiskContext(0, false, 0, 0, 0, true, false, 0, 0, "低风险", 1));

        DueDiligenceReport report = reporter.generateSafetyHold(snapshot, "常规复核", List.of("RISK_LEVEL_INVALID"));
    ...
    void anomalousSanctionSeverityProducesUncertaintyInsteadOfFalseWatchlistClaim() {
        InvestigationSnapshot snapshot = snapshot(new RiskContext(0, true, 0, 0, 0, true, false, 0, 0, "低风险", 1));
    ...
    private InvestigationSnapshot snapshot(RiskContext facts) {
        return new InvestigationSnapshot("snapshot-1", 1L, 1, Instant.parse("2026-08-01T00:00:00Z"),
                ...
                Map.of("尽职调查", List.of(new LegalDoc("LEGAL-VALID-1", "法规", "文号", "第一条", "内容"))), List.of("尽职调查"), facts,
                "legal-v1", "digest");
    }
