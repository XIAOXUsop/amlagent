...
/**
 * T01/T02：自动分析完成与最终结案分离。
 * 调查契约 v1 的案件自动分析后进入 HOLD 待人工处置；版本 0 保留自动完成行为。
 */
class DueDiligenceServiceTest {
    ...
    private final com.bank.aml.risk.RiskFactAssembler riskFactAssembler =
            mock(com.bank.aml.risk.RiskFactAssembler.class);
    ...
    void setUp() {
        ...
        when(cases.finishCase(any(), anyString(), anyInt(), any(), any(), any(), any(), any(),
                any(), any(), any(), any(), any(), org.mockito.ArgumentMatchers.anyBoolean())).thenReturn(1);
    }

    private DueDiligenceService service() {
        return new DueDiligenceService(cases, logs, executions, commands, metrics,
                agentFactory, ruleReporter,
                new GuardrailEngine(riskFactAssembler, ruleEngine),
                new com.bank.aml.agent.validation.AgentOutputValidator(),
                new FinalDecisionAssembler(),
                snapshotFactory, events, dataSource, alertRepository, alertAssembler, faultInjector,
                new PromptInjectionGuard(), new CostRouter(),
                false, false, streaming, new ObjectMapper().findAndRegisterModules(),
                archive, toolTraces, llmProperties, intake);
    }
    ...
    void contractV1LowRiskNormalOutputEndsInHoldWithConsistentManualReview() {
        ...
        when(snapshotFactory.create(eq(7L), eq(1), any(), eq((String) null), org.mockito.ArgumentMatchers.anyList()))
                .thenReturn(snapshot);
        stubAgent(snapshot);
        ...
        verify(cases).finishCase(eq(7L), eq("worker-1"), eq(1), statusCaptor.capture(),
                eq("低风险"), eq("低风险"), anyString(), anyString(), anyString(), eq("AGENT"),
                eq("case-7-v1"), eq("deepseek"), eq("test-model"), org.mockito.ArgumentMatchers.anyBoolean());
    ...
    void contractV0NormalOutputStillAutoCompletes() {
        ...
        verify(cases).finishCase(eq(7L), eq("worker-1"), eq(1), eq(CaseStatus.DONE),
                eq("低风险"), eq("低风险"), anyString(), anyString(), anyString(), eq("AGENT"),
                eq("case-7-v1"), eq("deepseek"), eq("test-model"), org.mockito.ArgumentMatchers.anyBoolean());
    ...
    void contractV1WithoutLinkedAlertsIsRejected() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> service().process(7L, "worker-1", 1, null))
                .isInstanceOf(com.bank.aml.common.exception.NonRetryableWorkflowException.class)
    ...
    void startingInvestigationBeyondCapacityIsRejectedBeforeEnqueue() {
        CaseEntity entity = caseEntity(1);
        when(cases.findByIdForUpdate(7L)).thenReturn(Optional.of(entity));
        when(alertRepository.countByCaseIdAndStatus(7L,
                com.bank.aml.investigation.AlertStatus.LINKED)).thenReturn(9L);

        org.assertj.core.api.Assertions.assertThatThrownBy(() -> service().trigger(7L))
    ...
    private InvestigationAlertSnapshot alertSnapshot() {
        return new InvestigationAlertSnapshot(11L, "ALERT-A", "RULE-001", "PROFILE_MISMATCH",
                "交易与客户画像不匹配", LocalDateTime.of(2026, 8, 1, 10, 0), 0);
    }

    private InvestigationSnapshot snapshot(int contractVersion) {
        CustomerProfile customer = customer();
        RiskContext riskFacts = new RiskContext(0, false, 0, 0, 0,
                true, true, 0, 0, "低风险", 1);
        return new InvestigationSnapshot("case-7-v1", 7L, 1, Instant.parse("2026-08-19T00:00:00Z"),
                customer, contractVersion >= 1 ? List.of(alertSnapshot()) : List.of(),
                List.of(), List.of(), List.of(), List.of(), java.util.Map.of(),
                List.of("尽职调查"), riskFacts, "v1",
    ...
    private void stubAgent(InvestigationSnapshot snapshot) {
        ...
        com.bank.aml.tools.SnapshotToolSuite tools = mock(com.bank.aml.tools.SnapshotToolSuite.class);
    ...
    private AgentAnalysis lowRiskAnalysis() {
        ...
        return new AgentAnalysis(
                ...
                "股权结构清晰，受益所有人为自然人",
                List.of(),
                List.of("依据客户尽职调查监管要求完成持续监测"),
                List.of("暂无新增异常交易模式"),
                "基于冻结证据维持常规持续监测，无需升级处置",
                List.of("交易画像：近期交易笔数与金额稳定"),
                false,
