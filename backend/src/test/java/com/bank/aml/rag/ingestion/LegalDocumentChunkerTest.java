...
class LegalDocumentChunkerTest {
    ...
    void parsesEffectivePeriodAndAccessPolicyFromControlledHeader() throws Exception {
        ...
        Files.writeString(file,
                "<!-- rag:jurisdiction=CN;effectiveFrom=2025-01-01;effectiveTo=2027-12-31;"
                        + "accessScopes=AML_INTERNAL;securityStatus=TRUSTED -->\n"
                        + "# 内部规则\n\n## 第一条 调查要求\n\n调查人员应核验交易背景和资金来源。\n");
    ...
    void splitsArticlesIntoParagraphAndItemLevelsWithManifestProvenance() throws Exception {
        ...
        var article3 = chunks.stream().filter(c -> c.text().contains("第三条 报告标准")).findFirst().orElseThrow();
        ...
        var article5 = chunks.stream().filter(c -> c.text().contains("第五条 保存期限")).findFirst().orElseThrow();
