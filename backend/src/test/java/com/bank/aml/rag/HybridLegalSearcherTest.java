...
/**
 * 混合检索 RRF 融合逻辑单元测试：验证两路召回（向量 + 关键词）的倒数排序融合、 按 evidenceId 去重、跨路命中排前、以及 topK 截断。不依赖真实
 * PGVector。
 */
class HybridLegalSearcherTest {
    ...
    private final HybridLegalSearcher hybrid = new HybridLegalSearcher(vector, keyword, TestClocks.FIXED);
    ...
    void rerfFusionBoostsDocHitInBothRanks() {
        ...
        when(vector.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.dense(1, 0.9, a), SearchHit.dense(2, 0.8, b)));
        ...
        // 去重：A 只出现一次
        assertThat(result).extracting(LegalDoc::evidenceId).containsOnlyOnce("A").contains("B");
    }

    @Test
    void preservesDenseAndLexicalScoresWhenSameEvidenceHitsBothChannels() {
        LegalDoc a = doc("A");
        when(vector.searchScored(any(RetrievalRequest.class), eq(15))).thenReturn(List.of(SearchHit.dense(1, 0.91, a)));
        when(keyword.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.lexical(2, 4.5, a, List.of("命中标题"))));

        SearchHit hit = hybrid
            .searchScored(
                    new RetrievalRequest("q", "q", TestClocks.FIXED.instant(), "CN", Set.of("PUBLIC_LEGAL"), 5, 0), 5)
            .getFirst();
    ...
    void strongFieldedLexicalMatchCanBeatDenseOnlyNoise() {
        ...
        when(vector.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.dense(1, 0.90, denseNoise), SearchHit.dense(8, 0.70, exact)));
        when(keyword.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.lexical(1, 12.0, exact, List.of("命中文号", "命中条号"))));
    ...
    void resultsOrderedByDescendingRrfScore() {
        when(vector.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.dense(1, 0.9, doc("A")), SearchHit.dense(2, 0.8, doc("B")),
                    SearchHit.dense(3, 0.7, doc("C"))));
        when(keyword.searchScored(any(RetrievalRequest.class), eq(15))).thenReturn(List.of());
        List<LegalDoc> result = hybrid.search("q", 5);
        assertThat(result).extracting(LegalDoc::evidenceId).containsExactly("A", "B", "C");
    }
    ...
    void returnsOnlyTopK() {
        when(vector.searchScored(any(RetrievalRequest.class), eq(10))).thenReturn(List.of(
                SearchHit.dense(1, 0.9, doc("A")), SearchHit.dense(2, 0.8, doc("B")), SearchHit.dense(3, 0.7, doc("C")),
                SearchHit.dense(4, 0.6, doc("D")), SearchHit.dense(5, 0.5, doc("E"))));
    ...
    void skipsDocsWithoutEvidenceId() {
        when(vector.searchScored(any(RetrievalRequest.class), eq(15)))
            .thenReturn(List.of(SearchHit.dense(1, 0.9, doc("X")),
