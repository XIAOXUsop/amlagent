...
class LegalQueryAnalyzerTest {
    ...
    void expandsBusinessSynonymsWithoutUsingAnLlm() {
        assertThat(analyzer.terms("公司账户一天累计划转200万元，多久报告")).contains("非自然人客户", "当日累计", "报告时限", "200万元");
        assertThat(analyzer.terms("复杂企业的 UBO 如何识别")).contains("受益所有人");
    }

    @Test
    void parsesAuthorityFieldsAndIntentDeterministically() {
        LegalQueryAnalyzer.ParsedQuery parsed = analyzer.parse("中国人民银行令〔2016〕第3号 第三条 大额交易 200万元 应当报告");
    ...
    void classifiesHighRiskDisposalAndGeneralKnowledgeIntents() {
        ...
        assertThat(analyzer.parse("什么是洗钱").intent()).isEqualTo(LegalQueryAnalyzer.QueryIntent.GENERAL_KNOWLEDGE);
        assertThat(analyzer.parse("客户身份资料至少保存几年").intent()).isEqualTo(LegalQueryAnalyzer.QueryIntent.REGULATION_FACT);
