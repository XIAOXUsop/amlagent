...
/**
 * 健康监控与自动恢复逻辑单元测试：验证 StreamHealthMonitor 对消费者停滞 / 连接失败的 分级响应（告警抑制、强制重建），以及
 * WorkflowConsumer 的停滞判定。
 * <p>
 * 纯内存测试（Mockito），不依赖 Redis。
 */
class StreamHealthMonitorTest {
    ...
    void sustainedStallRebuildsAfterRecoverThreshold() {
        ...
        verify(consumer, atLeast(1)).restartIfNeeded(true);
    }
