 ...
 * 本地确定性制裁名单匹配器。
 * <p>
 * 数据源的 LIKE 查询只负责召回候选，本类负责“证件号、姓名别名、主体类型”核验， 避免把包含客户姓名的公司条目直接当成自然人命中。实现不把客户身份数据发送到第三方服务。
 */
@Component
public class SanctionMatchScorer {
    ...
    private static final Pattern COMPANY_MARKERS = Pattern
        .compile("公司|有限公司|集团|银行|基金|TRADING|COMPANY|\\bCO\\b|\\bLTD\\b|\\bINC\\b|\\bCORP\\b", Pattern.CASE_INSENSITIVE);

    private final RiskProperties.Sanction policy;

    public SanctionMatchScorer() {
        this(new RiskProperties());
    }

    @Autowired
    public SanctionMatchScorer(RiskProperties properties) {
        this.policy = properties.getSanction();
    }
    ...
    public SanctionCandidateMatch score(CustomerProfile customer, SanctionRecord candidate) {
        ...
        double similarity = candidateNames.stream().mapToDouble(name -> similarity(customerName, name)).max().orElse(0);
        boolean nameContained = !customerName.isEmpty()
                && candidateNames.stream().anyMatch(name -> name.contains(customerName) || customerName.contains(name));
        ...
        if (identityExact) {
            score = policy.getIdentityExactScore();
            ...
            score = nameExact ? policy.getIdentityConflictWithNameScore() : policy.getIdentityConflictScore();
            ...
            score = nameContained || nameExact ? policy.getSubjectTypeConflictScore() : 0;
            decision = SanctionMatchDecision.DISMISSED;
            ...
            score = policy.getExactNameScore();
            decision = SanctionMatchDecision.REVIEW_REQUIRED;
            reasons.add("NAME_ALIAS_EXACT");
            reasons.add("IDENTITY_MISSING");
            explanation = "姓名或别名完全一致，但名单缺少可交叉核验的证件号码";
        }
        else if (similarity >= policy.getHighNameSimilarity()) {
            score = policy.getHighNameSimilarityScore();
            decision = SanctionMatchDecision.REVIEW_REQUIRED;
            ...
            score = policy.getContainedNameScore();
            decision = SanctionMatchDecision.REVIEW_REQUIRED;
            reasons.add("NAME_CONTAINED_ONLY");
            reasons.add("IDENTITY_MISSING");
