 ...
 *   <li>影响评估四类：正常后续记录 / 需补充核验 / 否定旧支持事实 / 来源更正；
 *       未完成匹配时禁止标记"无影响"。</li>
 * </ul>
 */
@Service
public class RefundAuthorityService {
    ...
    /** 退款解释可提交性评估结果。 */
    public record RefundAdmissibility(
            boolean admissible,
            String blockerCode,
            String explanation
    ) {
    ...
    /**
     * 退款解释门禁（§6.3 决策表）：
     * 权限 ORIGINAL_PAYER_VERIFIED/BUYER_VERIFIED + 商业原因 VERIFIED → 可提交；
     * 原路但原因缺失（RF-07）→ 原因未解决阻断；
     * 权限 UNRESOLVED（RF-09）/CONTRADICTED（RF-10）→ 阻断解释成立与依赖排除；
    ...
    public RefundAdmissibility assess(RecipientAuthority authority, CommercialReason reason) {
        ...
        return switch (authority) {
            case ORIGINAL_PAYER_VERIFIED -> reasonOk
                    ? new RefundAdmissibility(true, null, "原路退款且商业原因已核验；可提交退款解释")
                    : new RefundAdmissibility(false, "REFUND_REASON_UNRESOLVED",
                    "原路退款不自动免责（RF-07）：退款商业原因未解决，不能仅凭原路排除");
            case BUYER_VERIFIED -> reasonOk
                    ? new RefundAdmissibility(true, null, "买方收款权限已核实且商业原因成立；可进入人工解释复核（RF-08）")
                    : new RefundAdmissibility(false, "REFUND_REASON_UNRESOLVED",
                    "退款商业原因未核验；权限与原因需分别成立（RF-08）");
            case UNRESOLVED -> new RefundAdmissibility(false, "RECIPIENT_AUTHORITY_UNRESOLVED",
                    "退款收款权限未知（RF-09）：同集团/买方身份本身不足以证明收款权限，"
                            + "需主体与账户归属核验；阻断退款解释成立");
            case CONTRADICTED -> new RefundAdmissibility(false, "RECIPIENT_AUTHORITY_CONTRADICTED",
                    "收款权限被否认/矛盾（RF-10）：保留反证与冲突，不覆盖旧授权原文；"
                            + "是否形成怀疑由复核员结合证据决定");
        };
    }
    ...
    public record ImpactInput(
            ...
            boolean scopeGap,
            boolean verificationLost
    ) {
    }

    /**
     * 确定性影响评估（RefundEvent → 关联原分配 → 依赖事实 → 影响）。
     * 未完成匹配时禁止"无影响"——四个类型至少落一个。
     */
    ...
    public static Map<String, String> branchSemantics() {
        return Map.of(
                "ORIGINAL_PATH", "正常原路退款：新事件解释可提交复核；R1 原文不改写",
                "TO_BUYER", "退给合同买方：先核实 B 收款权限及 P/B/A 结算安排；身份本身不足以证明权限",
                "TO_UNRELATED", "退给无关账户：形成权限/用途未解决事项；禁止自动沿用原排除结论");
    }
}
