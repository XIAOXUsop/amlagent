 ...
 * <p>
 * Guardrails 使用的结构化事实必须来自工具结果或数据质量元数据，不能依赖模型自行声明。 提供纯函数
 * {@link #assembleFrom(List, List, List, String)}，可从已冻结的原始数据计算风险事实，
...
public class RiskFactAssembler {
    ...
    public RiskFactAssembler(CustomerDataPort dataSource) {
        this(dataSource, new SanctionMatchScorer(), null, new RiskProperties());
    }

    public RiskFactAssembler(CustomerDataPort dataSource, SanctionMatchScorer sanctionMatchScorer) {
        this(dataSource, sanctionMatchScorer, null, new RiskProperties());
    }

    @Autowired
    public RiskFactAssembler(CustomerDataPort dataSource, SanctionMatchScorer sanctionMatchScorer,
            SanctionScreeningService sanctionScreeningService, RiskProperties properties) {
    ...
    public RiskContext assemble(CustomerProfile customer, String modelRiskLevel) {
        return assembleFrom(dataSource.transactionsOf(customer.id()), dataSource.shareholdingsOf(customer.id()),
                searchSanctions(customer), modelRiskLevel);
    }

    /**
     * 纯函数：从已冻结的原始数据计算 RiskContext，不访问数据源。 用于快照工厂在单次读取后派生风险事实，保证 Agent 与 Guardrails
     * 共享同一份数据。
     */
    public RiskContext assembleFrom(List<TransactionRecord> txns, List<ShareholdingRecord> shareholdings,
        ...
        int maxSeverity = hits.stream()
            .mapToInt(SanctionRecord::severity)
        ...
        long large = txns.stream().filter(t -> t.amount().compareTo(transactionPolicy.getLargeAmount()) >= 0).count();
        ...
        return new RiskContext(maxSeverity, sanctionHit, crossRatio, nightRatio, large, dataComplete, riskExplained,
                patternSeverity, uboRiskSeverity, modelLevel, RiskLevel.fromLabel(modelLevel).code());
    }

    /**
     * 检索制裁名单（按姓名 + 证件号），只把高置信候选送入 Guardrail。 数据库模糊召回产生的低分候选仍可通过制裁筛查接口查看，但不会直接触发确定性制裁规则。
     */
    ...
    private int assessPatternSeverity(List<TransactionRecord> txns) {
        ...
        return maxSameAmount >= transactionPolicy.getRepeatedAmountCount() ? 2 : 0;
    }

    /** 受益所有人风险：境外/信托股东 → 2（无法核实）；关联公司/L2 → 1（材料待更新）；否则 0 */
    private int assessUboRisk(List<ShareholdingRecord> shareholdings) {
        boolean hasOffshore = shareholdings.stream()
            .anyMatch(s -> s.holder().toUpperCase().contains("LTD") || s.holder().toUpperCase().contains("TRUST")
