...
public class RiskRuleSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        seedOrUpgrade("SANCTION_LEVEL_1", 2, "一级制裁名单命中（强制转人工）", 10, "sanction.maxSeverity == 1", "高风险", "MANUAL_REVIEW",
                "命中 OFAC SDN 一级制裁名单，必须强制高风险并转人工复核");
        seedOrUpgrade("SANCTION_OTHER", 2, "其他制裁/可疑名单命中", 20, "sanction.maxSeverity >= 2", "高风险", "AUTO_DONE",
                "命中国内制裁或可疑交易名单，强制高风险");
        seedOrUpgrade("DATA_INCOMPLETE", 1, "交易数据不完整", 30, "transaction.dataComplete == false", "中风险", "MANUAL_REVIEW",
                "关键交易数据缺失时不能自动得出低风险结论，必须转人工补充核验");
        seedOrUpgrade("UBO_UNVERIFIED", 1, "受益所有人无法可靠核实", 40, "corporate.uboRiskSeverity >= 2", "高风险", "MANUAL_REVIEW",
                "最终受益所有人信息冲突或无法核实时，限制自动审批并转人工");
        seedOrUpgrade("TXN_PATTERN_HIGH", 1, "高严重度可疑交易模式", 50, "transaction.patternSeverity >= 2", "高风险", "AUTO_DONE",
        ...
        seedOrUpgrade("TXN_ABNORMAL", 2, "未解释的跨境+夜间高频异常", 60,
                ...
                "高风险", "AUTO_DONE", "高频跨境与夜间交易叠加，风险特征显著");
        seedOrUpgrade("UBO_DOCUMENT_INCOMPLETE", 1, "受益所有人材料待更新", 70, "corporate.uboRiskSeverity == 1", "中风险",
                "AUTO_DONE", "现有资料可识别受益所有人但最新证明不完整，应补充材料并加强监测");
        seedOrUpgrade("TXN_MODERATE", 2, "需加强监测的交易模式", 80, "transaction.patternSeverity == 1", "中风险", "AUTO_DONE",
    ...
    private void seedOrUpgrade(String code, int version, String name, int priority, String expr, String target,
