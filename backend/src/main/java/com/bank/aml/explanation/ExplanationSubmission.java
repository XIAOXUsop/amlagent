...
/**
 * 不可变单元提交（v2 计划 §9）：一次人工提交冻结问题、事实、材料、未知与建议。 payload
 * 一经写入不得修改；状态变化（WITHDRAWN/STALE/SUPERSEDED）通过字段与审计事件表达。
 */
@Entity
...
public class ExplanationSubmission {
    ...
    public ExplanationOutcome getOutcome() {
        return outcome;
    }

    public void setOutcome(ExplanationOutcome value) {
        outcome = value;
    }

    public boolean isSuspicionBasisComplete() {
        return suspicionBasisComplete;
    }

    public void setSuspicionBasisComplete(boolean value) {
        suspicionBasisComplete = value;
    }

    public boolean isCriticalUnknown() {
        return criticalUnknown;
    }

    public void setCriticalUnknown(boolean value) {
        criticalUnknown = value;
    }

    public boolean isUnresolvedDisclosed() {
        return unresolvedDisclosed;
    }

    public void setUnresolvedDisclosed(boolean value) {
        unresolvedDisclosed = value;
    }
    ...
    public void setSupersededReason(String value) {
