...
/**
 * 案件层决策表（v2 计划 §7.2）：检查建议及依据是否具备可复核条件，不自动化业务判断。
 * 与 docs/plans/examples/check-rapid-goods-v2-plan.mjs 的 evaluateDesign 保持同一口径；
...
public final class ExplanationDecisionRules {
    ...
    public record CaseContext(
            boolean caseStatusHold,
            boolean scopeEnumerated,
            boolean adoptedFactsUsable,
            boolean reviewBasisCurrent,
            boolean policyApplicable,
            boolean reviewerIndependent,
            boolean otherScenarioGateSatisfied,
            boolean hasOpenDecisionSupportEdd,
            boolean obligationTransferReady,
    ...
    public static Decision evaluate(CaseContext context, List<UnitAssessment> units) {
        boolean eligible = context.caseStatusHold()
                ...
                && !units.isEmpty() && units.stream().allMatch(UnitAssessment::assessmentValid);
        boolean taskGate = !context.hasOpenDecisionSupportEdd()
                || (context.obligationTransferReady() && context.continuationPlanReady());
        var explained = new java.util.ArrayList<UnitAssessment>();
        for (UnitAssessment unit : units) {
            if (unit.outcome() == ExplanationOutcome.EXPLAINED && explained(unit, context.continuationPlanReady())) {
                explained.add(unit);
            }
        }
        boolean canConfirmSuspicionPresent = units.stream().anyMatch(unit ->
                unit.outcome() == ExplanationOutcome.SUSPICIOUS && unit.suspicionBasisComplete());
        ...
        boolean canConfirm = eligible && taskGate
                && canConfirmSuspicionPresent && allEvaluableForSuspicion;
    ...
    private static boolean explained(UnitAssessment unit, boolean continuationPlanReady) {
        return unit.outcome() == ExplanationOutcome.EXPLAINED
                && !unit.criticalUnknown() && followupReady(unit, continuationPlanReady);
    }
