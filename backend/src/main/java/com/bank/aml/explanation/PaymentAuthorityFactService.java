 ...
 *
 * <p>每个授权事实分别记录：业务发生/生效时间 effectiveAt、系统得知时间 recordedAt、来源版本。
 * 核心规则：
 * <ul>
 *   <li>RF-18：授权覆盖校验使用付款发生时点——付款后才撤销，不否定"付款时已有效"（不自动否定 R1）；
 *       后续新退款是否需要新权限另行评估。</li>
 *   <li>RF-19：付款后上传的追认文件 → "事后确认"单独标记，不用上传日代替授权生效日；
 *       付款前权限未知保持待核验，不自动证明付款当时已授权。</li>
...
public class PaymentAuthorityFactService {
     ...
     *
     * <p>事实序列按 recordedAt 排序不代表业务顺序：effectiveFrom/effectiveTo 才是业务时间。
     * REVOKED 的生效日（effectiveFrom=撤销生效日）在付款日之后 → 付款时仍有效；
     * 撤销生效日 ≤ 付款日 → 付款时已无效。RATIFIED 不改变付款时点的评估（保持 UNKNOWN/原状态）。
     */
    public AuthorityValidity evaluateAtPayment(List<AuthorityFact> facts, LocalDate paymentDate) {
            ...
            switch (fact.factType() == null ? "" : fact.factType().toUpperCase()) {
                case "GRANTED" -> {
                    if (fact.effectiveFrom() != null && !fact.effectiveFrom().isAfter(paymentDate)) {
                        ...
                        grantExplanation = "授权 " + fact.authorityRef() + " 于 " + fact.effectiveFrom()
                                + " 生效，付款日 " + paymentDate + " 时已有效";
                    }
                }
                case "REVOKED" -> {
        ...
        if (grantedBeforePayment) {
            String suffix = revokedAfterPayment ? "；付款后授权被撤销，不影响付款时点有效性（RF-18），"
                    + "后续新付款/退款是否需要新权限另行评估" : "";
        ...
        return new AuthorityValidity("UNKNOWN", revokedAfterPayment, ratifiedAfterPayment,
                "付款前授权生效证据缺失，权限未知；付款后上传的追认文件属于事后确认（RF-19），"
                        + "不自动证明付款当时已获得授权，保持待核验");
    }
}
