...
public class ExplanationPolicyCatalog {
    ...
    private static final Set<String> SUPPORTED_GROUP_STATUS = Set.of(
            GROUP_RELATIONSHIP_CONFIRMED, GROUP_RELATIONSHIP_UNVERIFIED, GROUP_RELATIONSHIP_DENIED);
    ...
    public record Applicability(
        ...
        public Applicability(String businessRole, String paymentStage, boolean payerMatchesContractBuyer,
                             boolean payeeMatchesContractSeller, String contractNumber,
                             String deliveryDueDate) {
            this(businessRole, paymentStage, payerMatchesContractBuyer, payeeMatchesContractSeller,
                    contractNumber, deliveryDueDate, null);
        }
    }
            ...
            if (!input.payerMatchesContractBuyer()) {
                ...
                if (!SUPPORTED_GROUP_STATUS.contains(group)) {
                    return new Resolution(null,
                            "付款人与合同买方不一致时需声明集团关系状态（CONFIRMED/UNVERIFIED/DENIED）");
                }
                if (GROUP_RELATIONSHIP_DENIED.equals(group)) {
                    return new Resolution(null, "买方否认集团关系与代付授权：不适用集团代付配方，"
                            + "保留原始线索供人工研判");
                }
