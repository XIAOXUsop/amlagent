 ...
 *
 * <p>
 * 核心语义：同一材料可以有面向不同事实的多个核验动作；核验链按 "材料版本 + 核验对象（questionCode/claim 事实键）+ 事件序号"组织，
 * 链内最新有效状态决定该事实是否获得肯定支持——不能取整条材料的最后事件代表所有事实， 也不能让 Q1 的核验替 Q2 通过。
 *
 * <p>
 * 返回结构化 blocker（blockerCode + questionCode + artifactVersionId + remediation），
...
public class EvidenceAdmissibilityService {
    ...
    public record AdmissibilityResult(boolean admissible, String blockerCode, String questionCode,
            Long artifactVersionId, String remediation) {
     ...
     * <li>材料级通用核验（subjectFactKey=NULL）可作为兼容支持——前提是链内无更新的 UNRESOLVED/MISMATCH
     * 失去支持事件（§4.1：UNRESOLVED 不再提供该范围的肯定支持）。</li>
     * </ol>
     */
    public AdmissibilityResult assessQuestion(Long caseId, SubjectEvidence subject) {
        ...
        for (Long artifactVersionId : new LinkedHashSet<>(subject.artifactVersionIds())) {
            ...
            if ("MISMATCH".equals(artifact.getIntegrityStatus())) {
                return new AdmissibilityResult(false, "INTEGRITY_BLOCKED", questionCode, artifactVersionId,
                        "问题 " + questionCode + " 引用完整性不符（MISMATCH）的材料 " + artifact.getArtifactKey() + "；需更正来源");
            }
        ...
        if (!unavailable.isEmpty()) {
            return new AdmissibilityResult(false, "ARTIFACT_NOT_RESOLVED", questionCode,
                    ...
                    "问题 " + questionCode + " 引用的材料未取得内容：" + String.join("、", unavailable) + "；需重试来源或更换替代来源");
        }
        ...
        for (Long artifactVersionId : admissibleArtifacts) {
            ...
            if (status == SupportStatus.LOST) {
                return new AdmissibilityResult(false, "VERIFICATION_LOST", questionCode, artifactVersionId,
                        "问题 " + questionCode + " 的核验已失去支持（UNRESOLVED/MISMATCH）；" + "需重新核验该事实后才能形成可采用结论");
            }
        }
        return new AdmissibilityResult(false, "VERIFICATION_MISSING", questionCode,
                admissibleArtifacts.isEmpty() ? null : admissibleArtifacts.get(0), "问题 " + questionCode + " 引用的材料在"
                        + questionCode + " 事实上尚无有效 CONFIRMED 核验；" + "其它问题的核验不能替代本题（FR-01），请记录面向本事实的核验动作");
    }
    ...
    public SupportStatus effectiveSupport(Long artifactVersionId, String factKey) {
        ...
        // 两条查询各自有序，但直接拼接会让“较旧的通用核验”覆盖“较新的定向否定”。
        // 合并后必须按同一全局事件序（eventTime,id）重新排序；同一时刻由持久化 id 决定先后。
        chain.sort(Comparator
            .comparing(EvidenceVerificationEvent::getEventTime, Comparator.nullsFirst(Comparator.naturalOrder()))
            .thenComparing(EvidenceVerificationEvent::getId, Comparator.nullsFirst(Comparator.naturalOrder())));
        // 全局事件序升序 = 链内最新状态；只关心最后一个事件的 result。
