 ...
 *
 * <p>
 * 为什么不用自定义指标名：Prometheus 指标只能回答"调用了多少次、花了多少 token"，
 * 无法还原<b>单次调用</b>的上下文（哪次请求、哪个模型、耗时落在哪一段、失败在哪个环节）。 采用 OTel GenAI SIG
 * 正在收敛的标准命名后，本项目的模型调用追踪可以被任何兼容 OTel 的后端（Jaeger / Tempo / Datadog / Langfuse
 * 等）<b>直接理解</b>， 不需要为每个观测平台再写一套私有埋点。
 *
 * <p>
 * 标准仍在演进，属性名以 OTel 语义约定仓库为准；此处集中定义，升级时只改一处。
 */
public final class GenAiSemanticConventions {
    ...
    /**
     * 项目自定义扩展：调用用途（main_agent / summary / assistant）。 标准未定义该维度，但成本归属分析必须有它，故以
     * {@code aml.} 前缀扩展，不污染标准命名空间。
     */
