 ...
 *
 * <p>
 * 与 {@code ObservedChatModel}（Micrometer 指标）互补而非替代： 指标回答"总体调用了多少、P95 多少"，span
 * 回答"<b>这一次</b>调用发生在哪条链路、 用了哪个模型、token 与结束原因是什么"。两者配合才能既看大盘又能下钻单次调用。
 *
 * <p>
 * 装饰器形态与已有 {@code ObservedChatModel} 保持一致：purpose 由构造时显式传入， 不依赖 ThreadLocal（异步回调线程不会传播
 * ThreadLocal，会导致 purpose 丢失）。
 *
 * <p>
 * 注意：span 只记录模型名、token 数、结束原因等<b>元数据</b>， <b>绝不记录 prompt 或补全内容</b>——AML
 * 场景下那等同于把客户数据写进追踪后端。
 */
public final class GenAiTracingChatModel implements ChatModel {
    ...
    private void tagResponse(Span span, ChatResponse response) {
        ...
        if (finishReason != null) {
            span.tag(GenAiSemanticConventions.RESPONSE_FINISH_REASONS, finishReason.name().toLowerCase(Locale.ROOT));
        }
    }
