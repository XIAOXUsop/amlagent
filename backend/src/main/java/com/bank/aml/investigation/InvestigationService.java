...
public class InvestigationService {
    ...
    private static final Pattern EVIDENCE_REFERENCE = Pattern.compile("[A-Za-z0-9][A-Za-z0-9._:/-]{2,159}");
    ...
    private final ExplanationReadinessPort explanationReadiness;

    public InvestigationService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            InvestigationHypothesisRepository hypothesisRepository,
            InvestigationEvidenceLinkRepository evidenceRepository,
            AlertInvestigationCoverageRepository coverageRepository, InvestigationPlaybookCatalog playbooks,
            InvestigationReadinessEvaluator readinessEvaluator, AuditOutboxService auditOutbox) {
        this(caseRepository, alertRepository, hypothesisRepository, evidenceRepository, coverageRepository, playbooks,
                readinessEvaluator, auditOutbox, null);
    }
    ...
    public InvestigationService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            ...
            AlertInvestigationCoverageRepository coverageRepository, InvestigationPlaybookCatalog playbooks,
            InvestigationReadinessEvaluator readinessEvaluator, AuditOutboxService auditOutbox,
            ExplanationReadinessPort explanationReadiness) {
    ...
    public InvestigationHypothesis initializeForAlert(Long caseId, AmlAlert alert, String operator) {
        ...
        InvestigationHypothesis hypothesis = hypothesisRepository
            ...
            .orElseGet(() -> {
                ...
                created.setRequiredEvidenceTypes(
                        playbook.requiredEvidenceTypes().stream().map(Enum::name).collect(Collectors.joining(",")));
    ...
    public CaseInvestigationView get(Long caseId) {
        ...
        List<AlertView> alertViews = alertRepository.findByCaseIdOrderByOccurredAtAsc(caseId)
            .stream()
            .map(AlertView::from)
            .toList();
        List<AmlAlert> linkedAlerts = alertRepository.findByCaseIdOrderByOccurredAtAsc(caseId)
            .stream()
        ...
        if (caseEntity.getInvestigationContractVersion() >= 3) {
            ...
            result = explanationReadiness != null ? explanationReadiness.readinessForCase(caseId)
                    : new InvestigationReadinessEvaluator.Result(false, List.of("解释核验工作区未初始化"), List.of("解释核验工作区未初始化"),
                            List.of("解释核验工作区未初始化"));
        }
        else {
        ...
        return new CaseInvestigationView(caseEntity.getInvestigationContractVersion(), alertViews,
                hypotheses.stream().map(this::hypothesisView).toList(),
                coverage.stream().map(AlertCoverageView::from).toList(), result.readyForFinalReview(),
                result.generalBlockers(), result.confirmBlockers(), result.excludeBlockers());
    }

    @Transactional
    public InvestigationEvidenceView addEvidence(Long caseId, Long hypothesisId, String evidenceType,
            String evidenceReference, String stance, String findingSummary, String operator) {
        CaseEntity caseEntity = lockEditableCase(caseId);
        guardContractWrite(caseEntity);
        InvestigationHypothesis hypothesis = hypothesisRepository.findByIdAndCaseIdForUpdate(hypothesisId, caseId)
        ...
        auditOutbox.enqueue("INVESTIGATION_EVIDENCE:" + caseId + ":" + hypothesisId + ":" + reference, operator,
                "INVESTIGATION_EVIDENCE_ADD", "CASE", String.valueOf(caseId),
     ...
     * 确认或排除调查假设。
     * <p>
     * 与既有请求完全相同（状态与依据均未变化）时幂等返回当前视图，不递增版本； 实际修改了判断依据时递增版本，并在同一事务内把引用该假设的已决覆盖重置为 PENDING：
    ...
    public InvestigationHypothesisView updateHypothesis(Long caseId, Long hypothesisId, int expectedRevision,
        ...
        if (hypothesis.getRevision() != expectedRevision) {
            ...
            throw new InvestigationRevisionConflictException(InvestigationRevisionConflictException.TYPE_HYPOTHESIS,
                    hypothesisId, hypothesis.getRevision(),
                    "调查假设版本已变化（当前版本 " + hypothesis.getRevision() + "，请求基于版本 " + expectedRevision + "），请刷新后基于最新依据重新确认");
        }
        ...
        List<InvestigationEvidenceLink> evidence = evidenceRepository
            .findByHypothesisIdOrderByCreatedAtAsc(hypothesisId);
        Set<InvestigationEvidenceType> evidenceTypes = evidence.stream()
            .map(InvestigationEvidenceLink::getEvidenceType)
        ...
        EvidenceStance requiredStance = normalizedStatus == HypothesisStatus.CONFIRMED ? EvidenceStance.SUPPORTS
                : EvidenceStance.CONTRADICTS;
        if (evidence.stream().noneMatch(item -> item.getStance() == requiredStance)) {
            throw new IllegalStateException(
                    normalizedStatus == HypothesisStatus.CONFIRMED ? "确认假设至少需要一项支持证据" : "排除假设至少需要一项反向证据");
        }
        ...
        auditOutbox.enqueue("INVESTIGATION_HYPOTHESIS:" + hypothesisId + ":v" + saved.getRevision(), operator,
                "INVESTIGATION_HYPOTHESIS_DECIDE", "CASE", String.valueOf(caseId),
                "hypothesisId=" + hypothesisId + ",status=" + previousStatus + "->" + normalizedStatus + ",revision="
                        + previousRevision + "->" + saved.getRevision() + ",rationaleLen="
                        + normalizedRationale.length() + ",invalidatedCoverageIds=" + invalidated
                        + ",invalidatedCoverageCount=" + invalidated.size());
     ...
     * 更新预警覆盖结论。
     * <p>
     * 覆盖与假设版本分别校验：{@code expectedRevision} 校验覆盖自身的乐观锁， {@code expectedHypothesisRevision}
     * 校验判断依据（假设）的版本。缺省或过期的假设版本会得到 明确错误，不会默认采用当前版本而掩盖陈旧判断。保存成功时写入服务端锁定的当前假设版本。
     */
    @Transactional
    public AlertCoverageView updateCoverage(Long caseId, Long alertId, int expectedRevision, Long hypothesisId,
            Long expectedHypothesisRevision, String conclusion, String analysisSummary, String operator) {
        ...
        AlertInvestigationCoverage coverage = coverageRepository.findByAlertIdAndCaseIdForUpdate(alertId, caseId)
        ...
        if (coverage.getRevision() != expectedRevision) {
            ...
            throw new InvestigationRevisionConflictException(InvestigationRevisionConflictException.TYPE_COVERAGE,
                    alertId, coverage.getRevision(),
                    "预警覆盖结论版本已变化（当前版本 " + coverage.getRevision() + "，请求基于版本 " + expectedRevision + "），请刷新后重新确认");
        }
        AlertCoverageConclusion normalizedConclusion = parseEnum(AlertCoverageConclusion.class, conclusion, "预警覆盖结论");
        ...
        if (hypothesis.getRevision() != expectedHypothesisRevision) {
            ...
            throw new InvestigationRevisionConflictException(InvestigationRevisionConflictException.TYPE_HYPOTHESIS,
                    hypothesisId, hypothesis.getRevision(), "判断依据的假设已改判（当前版本 " + hypothesis.getRevision() + "，请求基于版本 "
                            + expectedHypothesisRevision + "），请刷新后基于最新依据重新确认");
        }
        ...
        auditOutbox.enqueue("ALERT_COVERAGE:" + alertId + ":" + expectedRevision, operator, "ALERT_COVERAGE_DECIDE",
                "ALERT", String.valueOf(alertId), "caseId=" + caseId + ",hypothesisId=" + hypothesisId
                        + ",hypothesisRevision=" + hypothesis.getRevision() + ",conclusion=" + normalizedConclusion);
    ...
    private void guardContractWrite(CaseEntity caseEntity) {
        ...
        if (version == 2) {
            throw new IllegalStateException("该案件启用了合理解释核验政策（v2）：" + "请通过预警单元提交入口更新调查结论，旧调查写入接口已关闭");
        }
    }
    ...
    public void validateReadyForReview(CaseEntity caseEntity, ReviewDecision decision) {
        ...
        if (version < 1 || decision == ReviewDecision.REQUEST_ENHANCED_DUE_DILIGENCE)
            return;
        List<AmlAlert> alerts = alertRepository.findByCaseIdOrderByOccurredAtAsc(caseEntity.getId())
            .stream()
            .filter(alert -> alert.getStatus() == AlertStatus.LINKED)
            .toList();
        List<InvestigationHypothesis> hypotheses = hypothesisRepository.findByCaseIdOrderByIdAsc(caseEntity.getId());
        List<AlertInvestigationCoverage> coverage = coverageRepository
            .findByCaseIdOrderByAlertIdAsc(caseEntity.getId());
        InvestigationReadinessEvaluator.Facts facts = new InvestigationReadinessEvaluator.Facts(
                caseEntity.getInvestigationContractVersion(), alerts, hypotheses, coverage,
                evidenceByHypothesis(caseEntity.getId()));
    ...
    public InvestigationReadinessEvaluator.Facts loadFactsForCase(Long caseId) {
        List<AmlAlert> alerts = alertRepository.findByCaseIdOrderByOccurredAtAsc(caseId)
            .stream()
            .filter(alert -> alert.getStatus() == AlertStatus.LINKED)
            .toList();
        List<InvestigationHypothesis> hypotheses = hypothesisRepository.findByCaseIdOrderByIdAsc(caseId);
    ...
    /**
     * 只有调查事实尚未形成时才允许重新划分案件边界。否则无法证明已有证据究竟属于 被拆出的预警还是仍留在原案件中的其他预警。
     */
    public void validateCanSplit(Long sourceCaseId) {
        ...
        if (hypothesisRepository.findByCaseIdOrderByIdAsc(sourceCaseId)
            .stream()
            .anyMatch(item -> item.getStatus() != HypothesisStatus.OPEN)) {
            throw new IllegalStateException("案件已有已决调查假设，不能再拆分预警");
        }
        if (coverageRepository.findByCaseIdOrderByAlertIdAsc(sourceCaseId)
            .stream()
    ...
    public void removeUnusedOpenHypotheses(Long sourceCaseId) {
        Set<Long> referenced = coverageRepository.findByCaseIdOrderByAlertIdAsc(sourceCaseId)
            .stream()
            .map(AlertInvestigationCoverage::getHypothesisId)
            .filter(Objects::nonNull)
            .collect(Collectors.toSet());
        hypothesisRepository.findByCaseIdOrderByIdAsc(sourceCaseId)
            .stream()
    ...
    private InvestigationReadinessEvaluator.Facts loadFacts(CaseEntity caseEntity, List<AmlAlert> alerts,
            List<InvestigationHypothesis> hypotheses, List<AlertInvestigationCoverage> coverage) {
        return new InvestigationReadinessEvaluator.Facts(caseEntity.getInvestigationContractVersion(), alerts,
                hypotheses, coverage, evidenceByHypothesis(caseEntity.getId()));
    }

    private InvestigationHypothesisView hypothesisView(InvestigationHypothesis hypothesis) {
        return new InvestigationHypothesisView(hypothesis.getId(), hypothesis.getCaseId(), hypothesis.getScenarioCode(),
                hypothesis.getHypothesisCode(), hypothesis.getTitle(), hypothesis.getInvestigationQuestion(),
                requiredEvidenceTypes(hypothesis), hypothesis.getStatus(), hypothesis.getRationale(),
                hypothesis.getRevision(), hypothesis.getCreatedBy(), hypothesis.getUpdatedBy(),
                UtcTimestamp.from(hypothesis.getCreatedAt()), UtcTimestamp.from(hypothesis.getUpdatedAt()),
                evidenceRepository.findByHypothesisIdOrderByCreatedAtAsc(hypothesis.getId())
                    .stream()
                    .map(InvestigationEvidenceView::from)
                    .toList());
    }
    ...
    private List<InvestigationEvidenceType> requiredEvidenceTypes(InvestigationHypothesis hypothesis) {
        ...
        return Arrays.stream(hypothesis.getRequiredEvidenceTypes().split(","))
            .map(value -> InvestigationEvidenceType.valueOf(value.trim()))
            .toList();
    }
