 ...
 * <p>
 * 对用户可控输入（如预警规则描述）做规则扫描，检测常见的提示注入模式。 与 prompt 层隔离指令、Guardrails 输出兜底共同构成三层防护： 代码检测（确定性）+
 * prompt 隔离 + 规则兜底。
 */
@Component
public class PromptInjectionGuard {
    ...
    private static final List<Pattern> INJECTION_PATTERNS = List.of(
            ...
            Pattern.compile("ignore.{0,12}(previous|above|prior)?\\s*(instructions|prompt|rules|system)",
    ...
    public InjectionResult scanHighConfidence(String input) {
        ...
        List<Pattern> highConfidence = List.of(INJECTION_PATTERNS.get(0), INJECTION_PATTERNS.get(1),
