 ...
 * <p>
 * 不保存工具参数明文（姓名/证件号/原始法规 query）与完整结果，避免敏感字段落库。
 */
public record ToolExecutionTrace(String toolName, boolean success, boolean argumentValid, long durationMs,
        String errorCode, String resultDigest, List<String> evidenceIds) {
    ...
    public static ToolExecutionTrace ok(String toolName, long durationMs, String resultDigest,
    ...
    public static ToolExecutionTrace invalidArgument(String toolName, long durationMs) {
        return new ToolExecutionTrace(toolName, false, false, durationMs, "ARGUMENT_VALIDATION_FAILED", null,
                List.of());
    }
