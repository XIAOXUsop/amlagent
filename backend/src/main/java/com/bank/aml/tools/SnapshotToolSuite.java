...
/**
 * 绑定冻结快照的只读工具套件：每个工单动态创建一份，四个工具只从 {@link InvestigationSnapshot} 读取 交易 / 股权 /
 * 制裁事实与预检索法规证据，不再访问 {@code CustomerDataPort} 或可变 RAG 索引。
 * <p>
 * 与评测模块 {@code AgentEvalFixtureTools} 的逐案例工具思路一致，保证 Agent 推理与 Guardrails
...
public class SnapshotToolSuite {
    ...
    public SnapshotToolSuite(InvestigationSnapshot snapshot) {
        this(snapshot, new RiskProperties().getTransaction());
    }

    public SnapshotToolSuite(InvestigationSnapshot snapshot, RiskProperties.Transaction transactionPolicy) {
        this.snapshot = Objects.requireNonNull(snapshot, "snapshot must not be null");
        this.transactionPolicy = Objects.requireNonNull(transactionPolicy, "transactionPolicy must not be null");
    }
    ...
    public String transactionProfile(@P("客户编号，如 C001") String customerId) {
        return record("transactionProfile", () -> {
            ...
            return TransactionTool.format(snapshot.transactions(), customerId, transactionPolicy);
        });
    }
    ...
    private String record(String toolName, Supplier<String> invocation) {
        long start = System.nanoTime();
        try {
    ...
    /**
     * 法规查询校验：query 必须命中快照冻结的任一法规关键词（不区分大小写）， 与评测工具契约一致，防止模型乱写查询词导致报告引用与工单主题无关的法规。
     */
    private List<String> requireLegalQuery(String query) {
        ...
        List<String> matched = snapshot.legalKeywords()
            .stream()
