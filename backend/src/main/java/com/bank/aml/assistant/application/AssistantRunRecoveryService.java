...
public class AssistantRunRecoveryService {
    ...
    public AssistantRunRecoveryService(AssistantProperties properties, AssistantRunRepository runs,
            AssistantRunStateService state, Clock clock) {
    ...
    public void recoverTimedOutRuns() {
        ...
        LocalDateTime cutoff = LocalDateTime.now(clock)
            .minusSeconds(properties.getRunTimeoutSeconds() + properties.getRecoveryGraceSeconds());
        ...
        if (recovered > 0)
            log.info("AI 小助已终结失联 run 数量={}", recovered);
    }
