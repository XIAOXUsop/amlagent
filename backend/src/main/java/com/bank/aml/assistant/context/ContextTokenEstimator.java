 ...
 *
 * <p>
 * 抽象成接口是为了让预算层不绑死在某一种分词器上；默认实现在 {@link DeterministicTokenEstimator}（零依赖、纯函数）。
 *
 * <p>
 * 刻意**不**调用 provider 的 {@code /tokenize} 端点做精确计数：那会让压缩变成
 * 非确定性且依赖网络，直接违反"全离线可测"与"同输入同输出"两条硬约束。 代价是估算偏保守（见默认实现的系数），会略早触发压缩。
 */
