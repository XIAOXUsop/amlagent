...
public class AssistantRunOrchestrator {
    ...
    public AssistantRunOrchestrator(AssistantConversationService conversations, AssistantRunStateService state,
            ...
            CustomerAssistantSnapshotFactory snapshots, AssistantSnapshotArchiveService snapshotArchive,
            AssistantHistoryLoader history, CustomerAssistantAgentFactory agents, ConversationLeaseService leases,
    ...
    public AssistantConversationService.AcceptedRun submitMessage(String conversationId, String operatorUsername,
        ...
        String persistedInput = decision.sanitizedInput() == null || decision.sanitizedInput().isBlank() ? "[请求需要澄清]"
    ...
    private void execute(String runId, AssistantInputGuard.InputDecision decision) {
        ...
        try {
            ...
            var snapshot = snapshots.create(runId, context.conversation(), decision.sanitizedInput(),
                    decision.intent());
            ...
            agentWithTools.agent()
                .chat(context.conversation().getId(), decision.sanitizedInput())
            ...
            if (modelError.get() != null) {
                ...
                log.warn("AI 小助模型调用失败 runId={} type={}", runId, error.getClass().getSimpleName(), error);
                List<AssistantToolTrace> toolTraces = agentWithTools.tools().traces();
                if (isToolRoundLimit(error)
                        && toolTraces.stream().anyMatch(trace -> "SUCCESS".equals(trace.status()))) {
            ...
            if (!validation.valid()) {
                if (toolTraces.stream().anyMatch(trace -> "SUCCESS".equals(trace.status()))) {
                    ...
                    var fallbackValidation = outputGuard.validate(snapshot, fallback);
                    if (fallbackValidation.valid()) {
                        log.warn("AI 小助模型输出未通过校验，已切换确定性只读回答 runId={} violations={}", runId, validation.violations());
                ...
                state.block(runId, BLOCKED_MESSAGE, String.join(",", validation.violations()), elapsedMs(started),
                        toolTraces);
            ...
            log.warn("AI 小助执行失败 runId={} type={}", runId, exception.getClass().getSimpleName(), exception);
            ...
            if (lease != null) {
                try {
                    ...
                    log.warn("释放助手运行租约失败，将依赖 TTL 回收: lease={}", lease, releaseFailure);
                }
            }
        }
    }
    ...
    private void safeFail(String runId, String code, long started,
        ...
        try {
            List<AssistantToolTrace> toolTraces = agentWithTools == null ? List.of() : agentWithTools.tools().traces();
    ...
    private String withLegalDisclaimer(CustomerAssistantSnapshot snapshot, String answer) {
        ...
        return "当前知识库没有找到足够可靠的法规依据" + reason + "，以下仅能提供客户事实分析，不能给出确定法律结论。\n\n" + answer;
    }

    /**
     * 仅对已经通过完整输出护栏的正文做有节奏的 SSE 分块。 这不是模型 token 直通，断线后仍可通过 Redis Stream 重放并以 MySQL 终态对账。
     */
    ...
    private static boolean isToolRoundLimit(Throwable error) {
        ...
        while (current != null) {
            ...
            if (message != null && message.contains("maxToolCallingRoundTrips"))
                return true;
