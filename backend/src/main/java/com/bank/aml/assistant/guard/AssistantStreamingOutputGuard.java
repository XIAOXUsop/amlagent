...
/**
 * 流式安全缓冲：始终保留尾部窗口，与下一 chunk 合并检测， 防止身份证/账号被拆分为多个 token 后绕过单 chunk 检测。
 */
public class AssistantStreamingOutputGuard {
    ...
    public AssistantStreamingOutputGuard(SensitiveDataDetector sensitiveData) {
        this(sensitiveData, List.of());
    }

    public AssistantStreamingOutputGuard(SensitiveDataDetector sensitiveData, Collection<String> allowedEvidenceIds) {
        this.sensitiveData = sensitiveData;
        this.allowedEvidenceIds = allowedEvidenceIds == null ? List.of()
    ...
    public synchronized String accept(String delta) {
        ...
        if (pending.length() <= holdBackChars)
            return "";
