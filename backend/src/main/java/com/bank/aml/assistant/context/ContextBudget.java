 ...
 * 上下文预算。
 *
 * <p>总预算 = provider 上下文窗口 − 输出预留 − 工具往返预留 − system prompt − 本轮用户输入，
...
public record ContextBudget(
        int providerContextWindow,
        int outputReserve,
        int toolRoundReserve,
        int systemPromptTokens,
        int currentTurnTokens,
    ...
    public ContextBudget {
        if (outputReserve + toolRoundReserve >= providerContextWindow) {
            throw new IllegalArgumentException(
                    "输出预留与工具预留之和必须小于上下文窗口：" + (outputReserve + toolRoundReserve)
