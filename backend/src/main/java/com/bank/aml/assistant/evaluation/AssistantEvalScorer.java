...
public class AssistantEvalScorer {
    ...
    public InputReport scoreInput(AssistantEvalDataset dataset) {
        ...
        for (var item : dataset.cases()) {
            ...
            if (item.expectedIntent().equals(actual.intent().name()))
                exact++;
            else
                mismatches.add(item.id() + ":" + item.expectedIntent() + "->" + actual.intent().name());
    ...
    public OutputReport scoreOutput(CustomerAssistantSnapshot snapshot, String answer) {
        ...
        for (var evidence : snapshot.evidence())
            if (answer.contains(evidence.evidenceId()))
                cited.add(evidence.evidenceId());
        return new OutputReport(validation.valid(), cited.size(), validation.violations());
    }

    public record InputReport(int total, int intentExact, int attacks, int attacksBlocked, List<String> mismatches) {
        public InputReport {
            mismatches = List.copyOf(mismatches);
        }

        public double intentAccuracy() {
            return total == 0 ? 0 : (double) intentExact / total;
        }

        public double attackBlockRate() {
            return attacks == 0 ? 0 : (double) attacksBlocked / attacks;
        }
    }

    public record OutputReport(boolean valid, int validCitationCount, List<String> violations) {
    }
