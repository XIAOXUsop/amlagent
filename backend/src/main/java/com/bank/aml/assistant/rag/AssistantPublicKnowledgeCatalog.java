...
public class AssistantPublicKnowledgeCatalog {
    ...
    public AssistantPublicKnowledgeCatalog(ObjectMapper objectMapper, Clock clock) {
        this(objectMapper, clock, new RagProperties());
    }

    @Autowired
    public AssistantPublicKnowledgeCatalog(ObjectMapper objectMapper, Clock clock, RagProperties properties) {
        this.clock = clock;
    ...
    public List<AssistantEvidence> retrieve(String query, Instant asOfTime, int limit) {
        ...
        LocalDate asOf = (asOfTime == null ? clock.instant() : asOfTime).atZone(ZoneOffset.UTC).toLocalDate();
    ...
    private int score(String query, Document document) {
        ...
        for (String keyword : document.keywords()) {
            ...
            if (!term.isBlank() && (query.contains(term) || term.contains(query)))
    ...
    private void validate(Catalog value) {
        ...
        for (Document document : value.documents()) {
            if (document == null || document.id() == null || !document.id().matches("KB-[A-Z0-9-]{6,64}")
                    || !ids.add(document.id()))
                throw new IllegalStateException("公开知识 evidenceId 非法或重复");
            ...
            if (blank(document.title()) || blank(document.summary())
                    || document.summary().length() > evidenceSummaryMaxCharacters || blank(document.source())
                    || document.effectiveFrom() == null || document.keywords() == null
    ...
    public record Document(String id, AssistantEvidence.EvidenceType type, String title, String summary, String source,
            String sourceUrl, LocalDate effectiveFrom, List<String> keywords) {
        public Document {
