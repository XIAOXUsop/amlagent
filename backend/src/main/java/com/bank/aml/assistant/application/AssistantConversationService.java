...
public class AssistantConversationService {
    ...
    public AssistantConversationService(AssistantProperties properties, CustomerRepository customers,
            ...
            AssistantRunRepository runs, Clock clock) {
    ...
    public AssistantConversationEntity create(Long customerId, String operatorUsername) {
        ...
        AssistantConversationEntity conversation = AssistantConversationEntity.create(operator, customer,
                LocalDateTime.now(clock).plusDays(properties.getRetentionDays()));
        return conversations.save(conversation);
    }

    @Transactional
    public ConversationView createView(Long customerId, String operatorUsername) {
        return toView(create(customerId, operatorUsername));
    }
    ...
    public Page<AssistantConversationEntity> list(Long customerId, String operatorUsername, int page, int size) {
        ...
        return conversations.findByOperatorUsernameAndCustomerIdOrderByUpdatedAtDesc(requireOperator(operatorUsername),
                customerId, PageRequest.of(page, size));
    }

    @Transactional(readOnly = true)
    public Page<ConversationView> listViews(Long customerId, String operatorUsername, int page, int size) {
        return list(customerId, operatorUsername, page, size).map(AssistantConversationService::toView);
    }
    ...
    @Transactional(readOnly = true)
    public ConversationView getView(String conversationId, String operatorUsername) {
        return toView(get(conversationId, operatorUsername));
    }
    ...
    @Transactional(readOnly = true)
    public List<MessageView> messageViews(String conversationId, String operatorUsername) {
        return messages(conversationId, operatorUsername).stream().map(AssistantConversationService::toView).toList();
    }
    ...
    /**
     * 原子接受用户消息并创建 AI 占位消息/run。同一会话持有数据库悲观锁， 因而 sequenceNo 与 clientMessageId 在多实例下仍保持幂等。
     */
    @Transactional
    public AcceptedRun acceptMessage(String conversationId, String operatorUsername, String clientMessageId,
            String content) {
        requireEnabled();
        String operator = requireOperator(operatorUsername);
        ...
        long nextSequence = messages.findTopByConversationIdOrderBySequenceNoDesc(conversationId)
            ...
            .orElse(1L);
        AssistantMessageEntity user = messages
            .save(AssistantMessageEntity.user(conversationId, nextSequence, clientId, message));
        AssistantMessageEntity assistant = messages
            .save(AssistantMessageEntity.assistantPlaceholder(conversationId, nextSequence + 1));
        AssistantRunEntity run = runs
            .save(AssistantRunEntity.accepted(conversationId, user.getId(), assistant.getId()));
        conversation.touch();
        conversations.save(conversation);
    ...
    private void requireActive(AssistantConversationEntity conversation) {
        ...
        if (conversation.getStatus() == AssistantConversationStatus.EXPIRED
                || !conversation.getExpiresAt().isAfter(LocalDateTime.now(clock))) {
            conversation.expire();
            conversations.save(conversation);
    ...
    public record ConversationView(String id, Long customerId, String customerNo, String status, Instant createdAt,
            Instant updatedAt, Instant expiresAt) {
    }

    public record MessageView(String id, long sequenceNo, String role, String status, String resultType, String content,
            Instant createdAt, Instant completedAt) {
    }

    private static ConversationView toView(AssistantConversationEntity entity) {
        return new ConversationView(entity.getId(), entity.getCustomerId(), entity.getCustomerNoAtCreation(),
                entity.getStatus().name(), toInstant(entity.getCreatedAt()), toInstant(entity.getUpdatedAt()),
                toInstant(entity.getExpiresAt()));
    }

    private static MessageView toView(AssistantMessageEntity entity) {
        return new MessageView(entity.getId(), entity.getSequenceNo(), entity.getRole().name(),
                entity.getStatus().name(), entity.getResultType() == null ? null : entity.getResultType().name(),
                entity.content(), toInstant(entity.getCreatedAt()), toInstant(entity.getCompletedAt()));
    }
