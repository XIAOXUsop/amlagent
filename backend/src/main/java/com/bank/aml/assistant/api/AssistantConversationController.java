...
public class AssistantConversationController {
    ...
    public AssistantConversationController(AssistantConversationService conversations,
            AssistantRunOrchestrator orchestrator, AssistantRateLimiter rateLimiter, AssistantProperties properties) {
    ...
    public AssistantDtos.ConversationResponse conversation(@PathVariable String conversationId) {
        return AssistantDtos.ConversationResponse.from(conversations.getView(conversationId, operator()));
    }

    @GetMapping("/conversations/{conversationId}/messages")
    public List<AssistantDtos.MessageResponse> messages(@PathVariable String conversationId) {
        return conversations.messageViews(conversationId, operator())
            .stream()
            .map(AssistantDtos.MessageResponse::from)
            .toList();
    }
    ...
    public ResponseEntity<AssistantDtos.AcceptedRunResponse> submit(@PathVariable String conversationId,
        ...
        var accepted = orchestrator.submitMessage(conversationId, operator, request.clientMessageId(),
                request.content());
        return ResponseEntity.status(HttpStatus.ACCEPTED)
            .body(new AssistantDtos.AcceptedRunResponse(accepted.runId(), accepted.userMessageId(),
                    accepted.assistantMessageId(), "ACCEPTED", accepted.idempotentReplay()));
    }
    ...
    public record SubmitMessageRequest(
            @NotBlank @Size(max = 64) @Pattern(regexp = "[A-Za-z0-9_-]+",
                    message = "clientMessageId 格式无效") String clientMessageId,
