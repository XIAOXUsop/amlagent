...
public class CustomerAssistantToolSuite {
    ...
    private final int frozenKnowledgeResultLimit;
    ...
    public CustomerAssistantToolSuite(CustomerAssistantSnapshot snapshot, ObjectMapper objectMapper) {
        this(snapshot, objectMapper,
                new com.bank.aml.assistant.config.AssistantProperties().getFrozenKnowledgeResultLimit());
    }

    public CustomerAssistantToolSuite(CustomerAssistantSnapshot snapshot, ObjectMapper objectMapper,
            int frozenKnowledgeResultLimit) {
    ...
    public String getCurrentTransactionRiskProfile() {
        ...
        return execute("getCurrentTransactionRiskProfile", () -> success(snapshot.transactionRisk(), evidenceIds),
                evidenceIds);
    }
    ...
    public String getCurrentOwnershipRiskSummary() {
        ...
        return execute("getCurrentOwnershipRiskSummary", () -> success(snapshot.ownershipRisk(), evidenceIds),
                evidenceIds);
    }
    ...
    public String getCurrentSanctionRiskSummary() {
        ...
        return execute("getCurrentSanctionRiskSummary", () -> success(snapshot.sanctionRisk(), evidenceIds),
                evidenceIds);
    }

    @Tool("读取本次冻结快照中指定 evidenceId 的安全证据摘要")
    public String getCurrentEvidence(@P("必须是前述工具返回的 evidenceId") String evidenceId) {
        return execute("getCurrentEvidence",
                () -> snapshot.evidence()
                    .stream()
                    .filter(item -> item.evidenceId().equals(evidenceId))
                    .findFirst()
                    .map(item -> success(item, List.of(item.evidenceId())))
    ...
    public List<AssistantToolTrace> traces() {
    ...
    private String searchFrozen(String toolName, String query, AssistantEvidence.EvidenceType type) {
        ...
        List<AssistantEvidence> matches = snapshot.evidence()
            .stream()
            ...
            .limit(frozenKnowledgeResultLimit)
    ...
    /**
     * 工具异常只向模型返回结构化、脱敏且可恢复的信息。非法 evidenceId 仍会被 服务端拒绝，但不会把一次参数失误升级为整轮模型失败。
     */
    ...
    private String execute(String toolName, Supplier<String> invocation, List<String> evidenceIds) {
        ...
        try {
            ...
            traces.add(new AssistantToolTrace(seq, toolName, "INVALID_ARGUMENT", elapsedMs(started), null, List.of(),
                    "INVALID_ARGUMENT"));
            throw e;
        }
        catch (RuntimeException e) {
            traces.add(new AssistantToolTrace(seq, toolName, "FAILED", elapsedMs(started), null, List.of(),
    ...
    private List<String> evidenceIds(AssistantEvidence.EvidenceType type) {
        return snapshot.evidence()
            .stream()
            .filter(item -> item.type() == type)
            .map(AssistantEvidence::evidenceId)
            .toList();
    }
    ...
    private static int lexicalScore(String query, String document) {
        if (query.isBlank() || document.isBlank())
            return 0;
    ...
    private record ToolResponse(boolean ok, Object data, List<String> evidenceIds, String errorCode, String message) {
    }
