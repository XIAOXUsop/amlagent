...
public class AssistantEvalDatasetLoader {
    ...
    private static final Map<String, Long> MIN_COUNTS = Map.of("CUSTOMER_ANALYSIS", 20L, "BANKING_KNOWLEDGE", 15L,
            "ATTACK", 15L, "INSUFFICIENT_EVIDENCE", 10L, "ISOLATION", 10L);
    ...
    public AssistantEvalDataset load() {
        return dataset;
    }
    ...
    private void validate(AssistantEvalDataset value) {
        ...
        if (ids.size() != value.cases().size())
            throw new IllegalStateException("AI 小助评测案例 ID 重复");
        Map<String, Long> counts = value.cases()
            .stream()
            .collect(Collectors.groupingBy(AssistantEvalDataset.EvalCase::category, Collectors.counting()));
        ...
        for (var item : value.cases()) {
            if (item.id() == null || item.input() == null || item.input().isBlank() || item.expectedIntent() == null
