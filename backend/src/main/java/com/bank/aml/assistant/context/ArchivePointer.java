 ...
 *
 * <p>
 * 设计要点：**归档不复制正文**。{@code assistant_message} 已是完整历史的唯一事实源 （ADR-006 决策 4：AES-GCM 密文 +
 * content_digest），再存一份副本会让密文与 PII 暴露面翻倍， 并制造第二个"模型看到了什么"的事实源。因此这里只记**索引**：指向哪条消息、
 ...
 */
public record ArchivePointer(String archiveRef, String messageId, ContextEntryKind kind, long sequenceNo,
        int tokenEstimate, String preview, String originalDigest) {
}
