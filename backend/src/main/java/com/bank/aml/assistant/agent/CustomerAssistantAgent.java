...
public interface CustomerAssistantAgent {
    ...
    @SystemMessage("""
            ...
            给出法律结论（应当/必须/不得/可以 + 金额、期限、处罚等判断)时，必须在回答末尾附带声明块：
            ```json
            {"claims":[{"claimId":"C1","type":"LEGAL_REQUIREMENT","text":"说清楚法律责任或标准",
            "evidenceIds":["引用证据"],"supportSpans":["证据原文连续片段；数字与情态必须原文一致"]}]}
            ```
