...
public class CustomerAssistantSnapshotFactory {
    ...
    public CustomerAssistantSnapshot create(String runId, AssistantConversationEntity conversation, String question,
        ...
        SanctionRiskView sanctionView = new SanctionRiskView(facts.sanctionHit(), sanctions.size(), facts.maxSeverity(),
                sanctions.stream()
        ...
        return new CustomerAssistantSnapshot("assistant-" + runId, conversation.getId(), runId, dataSource.asOfTime(),
                customerView, transactionView, ownershipView, sanctionView, evidence,
                safe(dataSource.sourceSystem(), "UNKNOWN"), safe(dataSource.sourceVersion(), "unknown"), indexVersion,
                retrievalStatus, digest);
    }

    private RetrievalStatusView retrievalView(AssistantKnowledgeProvider.KnowledgeBundle bundle, String question) {
        if (bundle.retrievalStatus() == null)
            return RetrievalStatusView.NONE;
        return new RetrievalStatusView(bundle.retrievalStatus().name(), bundle.topSupportProbability(),
                safe(bundle.version(), "unavailable"), AssistantDigests.sha256(question), bundle.rejectedReasons(),
                bundle.traceId());
    }

    private TransactionRiskView transactionView(List<TransactionRecord> transactions, RiskContext facts) {
        ...
        List<String> currencies = transactions.stream()
            ...
            .sorted()
            .toList();
        List<String> regions = transactions.stream()
            .map(TransactionRecord::country)
            .filter(Objects::nonNull)
    ...
    private OwnershipRiskView ownershipView(List<ShareholdingRecord> records, int severity) {
        var relations = records.stream()
            .map(item -> new OwnershipRiskView.OwnershipRelationView(maskName(item.holder()), item.holderType(),
    ...
    private List<AssistantEvidence> evidence(AssistantCustomerView customer, TransactionRiskView tx,
            OwnershipRiskView ownership, SanctionRiskView sanction, String sourceSystem, String sourceVersion) {
        ...
        drafts.add(new EvidenceDraft(AssistantEvidence.EvidenceType.CUSTOMER_PROFILE, "客户画像",
                "客户类型=" + safe(customer.customerType(), "未知") + "，行业=" + safe(customer.industry(), "未知") + "，地区="
                        + safe(customer.region(), "未知") + "，状态=" + customer.status()));
        drafts.add(new EvidenceDraft(AssistantEvidence.EvidenceType.TRANSACTION_AGGREGATE, "交易聚合",
                "交易笔数=" + tx.transactionCount() + "，总额=" + tx.totalAmount() + "，夜间占比=" + tx.nightRatio() + "% ，跨境占比="
                        + tx.crossBorderRatio() + "% ，大额笔数=" + tx.largeTransactionCount()));
        ...
        drafts.add(new EvidenceDraft(AssistantEvidence.EvidenceType.SANCTION, "制裁筛查摘要",
                "是否命中=" + sanction.hit() + "，命中数=" + sanction.hitCount() + "，最高严重度=" + sanction.maxSeverity()));
    ...
    private String digest(AssistantCustomerView customer, TransactionRiskView tx, OwnershipRiskView ownership,
            SanctionRiskView sanction, List<AssistantEvidence> evidence) {
