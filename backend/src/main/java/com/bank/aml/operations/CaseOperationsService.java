...
public class CaseOperationsService {
    ...
    private static final List<CaseStatus> ACTIVE_STATUSES = List.of(CaseStatus.PENDING, CaseStatus.RUNNING,
            CaseStatus.RETRY_WAIT, CaseStatus.HOLD, CaseStatus.REPORT_PENDING, CaseStatus.FAILED);
    ...
    private final OperationsProperties policy;
    ...
    public CaseOperationsService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            EnhancedDueDiligenceRequestRepository eddRepository, CaseExecutionRepository executionRepository,
        ...
        this(caseRepository, alertRepository, eddRepository, executionRepository, reportRepository,
                new OperationsProperties(), clock);
    }

    @Autowired
    public CaseOperationsService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            EnhancedDueDiligenceRequestRepository eddRepository, CaseExecutionRepository executionRepository,
            SuspiciousTransactionReportRepository reportRepository, OperationsProperties policy, Clock clock) {
        this(caseRepository, alertRepository, eddRepository, executionRepository, reportRepository, null, null, null,
                new InvestigationReadinessEvaluator(), null, policy, clock);
    }
    ...
    CaseOperationsService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            ...
            InvestigationReadinessEvaluator readinessEvaluator, ExplanationReadinessPort explanationReadiness,
            Clock clock) {
        this(caseRepository, alertRepository, eddRepository, executionRepository, reportRepository,
                hypothesisRepository, coverageRepository, investigationEvidenceRepository, readinessEvaluator,
                explanationReadiness, new OperationsProperties(), clock);
    }
    ...
    CaseOperationsService(CaseRepository caseRepository, AmlAlertRepository alertRepository,
            ...
            InvestigationEvidenceLinkRepository investigationEvidenceRepository,
            InvestigationReadinessEvaluator readinessEvaluator, ExplanationReadinessPort explanationReadiness,
            OperationsProperties policy, Clock clock) {
    ...
    @Transactional(readOnly = true)
    public List<CaseOperationsView> queue(String username, String role, boolean overdueOnly, CasePriority priority,
        ...
        return active.stream()
            ...
            .sorted(Comparator.comparing(CaseOperationsView::overdue)
                .reversed()
                .thenComparing(CaseOperationsView::priorityScore, Comparator.reverseOrder())
                .thenComparing(CaseOperationsView::dueAt, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    private Map<Long, InvestigationReadinessEvaluator.Result> preloadReadiness(Collection<CaseEntity> cases) {
        List<Long> v2CaseIds = cases.stream()
            .filter(item -> item.getStatus() == CaseStatus.HOLD && item.getInvestigationContractVersion() == 2
        ...
        if (!v2CaseIds.isEmpty()) {
            ...
            Map<Long, InvestigationReadinessEvaluator.Result> merged = new HashMap<>(preloadV1Readiness(cases));
    ...
    private Map<Long, InvestigationReadinessEvaluator.Result> preloadV1Readiness(Collection<CaseEntity> cases) {
        List<Long> caseIds = cases.stream()
            .filter(item -> item.getStatus() == CaseStatus.HOLD && item.getInvestigationContractVersion() == 1)
            .map(CaseEntity::getId)
            .toList();
        ...
        Map<Long, List<InvestigationEvidenceLink>> evidenceByCase = new HashMap<>();
        for (InvestigationEvidenceLink link : investigationEvidenceRepository.findByCaseIdIn(caseIds)) {
        ...
        for (Long caseId : caseIds) {
            ...
            Map<Long, List<InvestigationEvidenceLink>> evidenceByHypothesis = new HashMap<>();
            for (InvestigationEvidenceLink link : evidenceByCase.getOrDefault(caseId, List.of())) {
                evidenceByHypothesis.computeIfAbsent(link.getHypothesisId(), key -> new ArrayList<>()).add(link);
            }
            results.put(caseId,
                    readinessEvaluator.evaluate(new InvestigationReadinessEvaluator.Facts(1, linkedAlerts(caseId),
                            hypotheses, coverageByCase.getOrDefault(caseId, List.of()), evidenceByHypothesis)));
        }
        return results;
    }
    ...
    CaseOperationsView view(CaseEntity caseEntity) {
        ...
        InvestigationReadinessEvaluator.Result readiness = caseEntity.getStatus() == CaseStatus.HOLD
                && caseEntity.getInvestigationContractVersion() == 1
                        ? readReadiness(caseEntity.getId())
                        : (caseEntity.getStatus() == CaseStatus.HOLD
                                && caseEntity.getInvestigationContractVersion() == 2 && explanationReadiness != null
    ...
    private CaseOperationsView view(CaseEntity caseEntity, InvestigationReadinessEvaluator.Result preloadedReadiness) {
        ...
        if (phaseClock.phase() == OperationPhase.INVESTIGATION && caseEntity.getStatus() == CaseStatus.HOLD
            ...
            reasons.addAll(preloadedReadiness.generalBlockers());
        }
        boolean overdue = phaseClock.dueAt() != null && !phaseClock.dueAt().isAfter(now);
        long minutesRemaining = phaseClock.dueAt() == null ? 0 : Duration.between(now, phaseClock.dueAt()).toMinutes();
        return new CaseOperationsView(caseEntity.getId(), caseEntity.getCustomerId(), caseEntity.getCustomerName(),
                caseEntity.getStatus(), assessment.priority(), assessment.score(),
                List.copyOf(new LinkedHashSet<>(reasons)), policyName("DETERMINISTIC_CASE_PRIORITY"),
                phaseClock.phase(), phaseClock.responsibleRole(), phaseClock.assignedTo(), phaseClock.assignedUnit(),
                UtcTimestamp.from(phaseClock.startedAt()), UtcTimestamp.from(phaseClock.dueAt()), overdue,
                minutesRemaining, phaseClock.policy(), UtcTimestamp.from(now));
    }
    ...
    private boolean visibleTo(CaseOperationsView item, String username, String role) {
        ...
        if ("ANALYST".equals(role)) {
            return item.phase() == OperationPhase.INVESTIGATION
                    || (item.phase() == OperationPhase.ENHANCED_DUE_DILIGENCE && username != null
                            && username.equals(item.assignedTo()));
        }
        return false;
    }

    private PriorityAssessment priority(CaseEntity caseEntity, List<AmlAlert> alerts) {
        int score = policy.getBaseScore();
        LinkedHashSet<String> reasons = new LinkedHashSet<>();
        Set<String> scenarios = alerts.stream()
            .map(AmlAlert::getScenarioCode)
            .filter(Objects::nonNull)
            .collect(Collectors.toSet());
        for (String scenario : scenarios) {
            switch (scenario) {
                case "SANCTIONS_WATCHLIST" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("名单身份核验场景");
                }
                case "STRUCTURING" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("疑似拆分交易规避监测");
                }
                case "RAPID_MOVEMENT" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("资金快进快出场景");
                }
                case "CROSS_BORDER_ANOMALY" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("异常跨境交易场景");
                }
                case "COMPLEX_OWNERSHIP" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("复杂受益所有权场景");
                }
                case "PROFILE_MISMATCH" -> {
                    score += policy.scenarioWeight(scenario);
                    reasons.add("交易与客户画像不匹配");
                }
                default -> reasons.add("其他监测场景");
            }
        }
        if (alerts.size() > 1) {
            score += Math.min(policy.getAdditionalAlertMaxBonus(),
                    (alerts.size() - 1) * policy.getAdditionalAlertWeight());
            reasons.add("同一客户聚合 " + alerts.size() + " 条有效预警");
        }
        if ("高风险".equals(caseEntity.getRiskLevel())) {
            score = Math.max(policy.getHighRiskMinimum(), score + policy.getHighRiskBonus());
            reasons.add("规则护栏后为高风险");
        }
        else if ("中风险".equals(caseEntity.getRiskLevel())) {
            score += policy.getMediumRiskBonus();
            reasons.add("规则护栏后为中风险");
        }
        if (caseEntity.getStatus() == CaseStatus.REPORT_PENDING) {
            score = Math.max(policy.getReportPendingMinimum(), score + policy.getReportPendingBonus());
            reasons.add("已确认可疑，等待完成报送");
        }
        if (caseEntity.getStatus() == CaseStatus.FAILED) {
            score += policy.getFailedCaseBonus();
            reasons.add("自动调查失败，需要人工恢复");
        }
        score = Math.min(policy.getMaximumScore(), score);
        CasePriority priority = score >= policy.getCriticalThreshold() ? CasePriority.CRITICAL
                : score >= policy.getHighThreshold() ? CasePriority.HIGH
                        : score >= policy.getMediumThreshold() ? CasePriority.MEDIUM : CasePriority.NORMAL;
        if (reasons.isEmpty())
            reasons.add("常规案件基线");
    ...
    private PhaseClock phaseClock(CaseEntity caseEntity, List<AmlAlert> alerts, CasePriority priority,
        ...
        if (caseEntity.getStatus() == CaseStatus.DONE) {
            return new PhaseClock(OperationPhase.COMPLETED, "COMPLETED", null, "已完成",
                    fallback(caseEntity.getReviewedAt(), caseEntity), null, policyName("SLA_COMPLETED"));
        }
        if (caseEntity.getStatus() == CaseStatus.HOLD) {
            ...
            if (latest != null && latest.getStatus() == EnhancedDueDiligenceStatus.OPEN) {
                return new PhaseClock(OperationPhase.ENHANCED_DUE_DILIGENCE, "ANALYST", latest.getAssignedTo(),
                        latest.getAssignedUnit(), latest.getRequestedAt(), latest.getDueAt(),
                        policyName("EDD_EXPLICIT_DUE_AT"));
            }
            ...
            if (!readiness.readyForFinalReview()) {
                ...
                LocalDateTime start = alerts.stream()
                    .map(AmlAlert::getCreatedAt)
                    .filter(Objects::nonNull)
                    .min(LocalDateTime::compareTo)
                    .orElse(caseEntity.getCreatedAt());
                return deadline(OperationPhase.INVESTIGATION, "ANALYST", null, "调查待补齐", fallback(start, caseEntity),
                        investigationHours(priority), policyName("RISK_BASED_INVESTIGATION_SLA"));
            }
            // 阶段起点只用可持久化事实：自动分析完成、有效假设/覆盖最后完成、最近 EDD 回传/撤销的最晚时间。
            LocalDateTime reviewStart = reviewPhaseStart(caseEntity, eddEvent);
            return deadline(OperationPhase.REVIEW, "REVIEWER", null, "人工复核队列", fallback(reviewStart, caseEntity),
                    reviewHours(priority), policyName("RISK_BASED_REVIEW_SLA"));
        }
        if (caseEntity.getStatus() == CaseStatus.REPORT_PENDING) {
            ...
            return deadline(OperationPhase.REPORTING, "REVIEWER", null, "可疑交易报告队列",
                    ...
                    policyName("RISK_BASED_REPORTING_SLA"));
        }
        ...
        LocalDateTime start = alerts.stream()
            ...
            .min(LocalDateTime::compareTo)
            .orElse(caseEntity.getCreatedAt());
        return deadline(OperationPhase.INVESTIGATION, "ANALYST", null, "反洗钱分析队列", fallback(start, caseEntity),
                investigationHours(priority), policyName("RISK_BASED_INVESTIGATION_SLA"));
    }

    private LocalDateTime latestExecutionCompletion(CaseEntity caseEntity) {
        return executionRepository.findByCaseIdOrderByStartedAtAsc(caseEntity.getId())
            .stream()
            .map(CaseExecution::getCompletedAt)
            .filter(Objects::nonNull)
    ...
    private InvestigationReadinessEvaluator.Result readReadiness(Long caseId) {
        if (hypothesisRepository == null || coverageRepository == null || investigationEvidenceRepository == null) {
        ...
        Map<Long, List<InvestigationEvidenceLink>> evidenceByHypothesis = new HashMap<>();
        for (InvestigationEvidenceLink link : investigationEvidenceRepository.findByCaseIdOrderByCreatedAtAsc(caseId)) {
            evidenceByHypothesis.computeIfAbsent(link.getHypothesisId(), key -> new ArrayList<>()).add(link);
        }
        return readinessEvaluator.evaluate(new InvestigationReadinessEvaluator.Facts(1, linkedAlerts(caseId),
                hypotheses, coverage, evidenceByHypothesis));
    }

    /**
     * 复核阶段起点：自动分析完成、有效假设/覆盖最后完成与最近 EDD 回传/撤销的最晚时间。 全部来自已持久化业务事实，不随查询时间漂移。
     */
        ...
        if (hypothesisRepository != null && coverageRepository != null) {
            start = maxTime(start,
                    hypothesisRepository.findByCaseIdOrderByIdAsc(caseEntity.getId())
                        ...
                        .map(InvestigationHypothesis::getUpdatedAt)
                        .filter(Objects::nonNull)
                        .max(LocalDateTime::compareTo)
                        .orElse(null));
            start = maxTime(start,
                    coverageRepository.findByCaseIdOrderByAlertIdAsc(caseEntity.getId())
                        .stream()
                        .filter(item -> item.getConclusion() != AlertCoverageConclusion.PENDING)
                        .map(AlertInvestigationCoverage::getUpdatedAt)
                        .filter(Objects::nonNull)
                        .max(LocalDateTime::compareTo)
                        .orElse(null));
        }
    ...
    private PhaseClock deadline(OperationPhase phase, String role, String assignedTo, String unit, LocalDateTime start,
            int hours, String policy) {
        return new PhaseClock(phase, role, assignedTo, unit, start, start.plusHours(hours), policy + "_" + hours + "H");
    }

    private String policyName(String suffix) {
        return policy.getPolicyVersion() + "_" + suffix;
    }

    private int investigationHours(CasePriority priority) {
        return policy.getSla().investigationHours(priority.name());
    }

    private int reviewHours(CasePriority priority) {
        return policy.getSla().reviewHours(priority.name());
    }

    private int reportHours(CasePriority priority) {
        return policy.getSla().reportHours(priority.name());
    }
