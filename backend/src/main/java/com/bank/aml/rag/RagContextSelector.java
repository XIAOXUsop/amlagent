...
public class RagContextSelector {
    ...
    public List<LegalDoc> select(List<LegalDoc> ranked, int maxPerDocument, int maxCharacters,
        ...
        for (LegalDoc candidate : ranked) {
            ...
            if (documentKey == null || documentKey.isBlank())
                documentKey = candidate.title();
            if (perDocument.getOrDefault(documentKey, 0) >= maxPerDocument)
                continue;
            if (selected.stream()
                .anyMatch(existing -> duplicate(existing.content(), candidate.content(), duplicateThreshold))) {
                continue;
            }
            int length = candidate.content() == null ? 0 : candidate.content().length();
            if (!selected.isEmpty() && characters + length > maxCharacters)
                continue;
