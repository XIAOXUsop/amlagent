...
public record RetrievalRequest(String query, String topic, Instant asOfTime, String jurisdiction,
        Set<String> accessScopes, int topK, double minRelevance, RetrievalTarget target, String specificVersion,
        CacheMode cacheMode) {
    /** 生产检索便捷构造：默认 ACTIVE 目标、NORMAL 缓存。 */
    public RetrievalRequest(String query, String topic, Instant asOfTime, String jurisdiction, Set<String> accessScopes,
            int topK, double minRelevance) {
        this(query, topic, asOfTime, jurisdiction, accessScopes, topK, minRelevance, RetrievalTarget.ACTIVE, null,
                CacheMode.NORMAL);
    }
    ...
    public RetrievalRequest {
        ...
        query = query.trim();
        if (query.length() > 512)
            throw new IllegalArgumentException("检索 query 过长");
        topic = topic == null ? "" : topic.trim();
        if (topic.length() > 128)
            throw new IllegalArgumentException("检索 topic 过长");
        if (asOfTime == null)
            throw new IllegalArgumentException("检索 asOfTime 不能为空");
        ...
        if (accessScopes.isEmpty())
            throw new IllegalArgumentException("检索访问范围不能为空");
        if (accessScopes.size() > 16
                || accessScopes.stream().anyMatch(scope -> scope == null || !scope.matches("[A-Z][A-Z0-9_]{1,63}"))) {
        ...
        if (minRelevance < 0 || minRelevance > 1)
            throw new IllegalArgumentException("minRelevance 必须在 0..1");
