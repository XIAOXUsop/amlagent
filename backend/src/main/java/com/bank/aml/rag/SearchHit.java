 ...
 * </ul>
 */
public record SearchHit(LegalDoc document, Integer denseRank, Double denseScore, Integer lexicalRank,
        Double lexicalScore, Double fusionScore, Double rerankScore, Double supportProbability,
        Set<RetrievalChannel> channels, List<String> matchReasons) {
    public SearchHit {
    ...
    public SearchHit reranked(int rank, double score) {
        ...
        return new SearchHit(document, denseRank, denseScore, lexicalRank, lexicalScore, fusionScore, score,
                supportProbability, merged, matchReasons);
    }

    /** 附上支持概率（校准后）。 */
    public SearchHit support(double probability) {
        return new SearchHit(document, denseRank, denseScore, lexicalRank, lexicalScore, fusionScore, rerankScore,
                probability, channels, matchReasons);
    }
    ...
    public SearchHit withReasons(List<String> extra) {
        ...
        if (extra != null)
            merged.addAll(extra);
        return new SearchHit(document, denseRank, denseScore, lexicalRank, lexicalScore, fusionScore, rerankScore,
                supportProbability, channels, merged);
    }
    ...
    public SearchHit mergeRecall(SearchHit other) {
        if (other == null)
            return this;
        if (!Objects.equals(document.evidenceId(), other.document.evidenceId())) {
        ...
        return new SearchHit(document, first(denseRank, other.denseRank), first(denseScore, other.denseScore),
    ...
    public static SearchHit dense(int rank, double score, LegalDoc doc) {
        return new SearchHit(doc, rank, score, null, null, null, null, null, Set.of(RetrievalChannel.DENSE), List.of());
    }

    public static SearchHit lexical(int rank, double score, LegalDoc doc, List<String> reasons) {
        return new SearchHit(doc, null, null, rank, score, null, null, null, Set.of(RetrievalChannel.LEXICAL),
    ...
    public SearchHit fused(double score, Set<RetrievalChannel> allChannels, List<String> reasons) {
        ...
        Set<String> mergedReasons = new LinkedHashSet<>(matchReasons);
        if (reasons != null)
            mergedReasons.addAll(reasons);
        return new SearchHit(document, denseRank, denseScore, lexicalRank, lexicalScore, score, rerankScore,
                supportProbability, merged, List.copyOf(mergedReasons));
    }
