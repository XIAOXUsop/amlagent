...
/**
 * RAG 检索缓存：装饰召回+精排搜索，用 Redis 缓存相同查询的检索结果（含各阶段分数 SearchHit）， 避免重复 embedding 与向量检索，降低 LLM
 * 应用的计算与延迟成本。
 * <p>
 * 评测/门禁通过 {@link CacheMode#BYPASS_READ_WRITE} 显式绕过，不读写生产缓存。
 * </p>
 */
@Component
@Primary
public class CachingLegalSearcher implements LegalDocumentSearcher {
    ...
    public CachingLegalSearcher(ReRankingLegalSearcher delegate, StringRedisTemplate redisTemplate,
            MetricsRecorder metrics, ObjectMapper objectMapper, long ttlMinutes,
            LegalIndexVersionProvider indexVersions, String embeddingModel, String rerankerVersion) {
        this(delegate, redisTemplate, metrics, objectMapper, ttlMinutes, indexVersions, embeddingModel, rerankerVersion,
                "hybrid-rrf-v7");
    }

    @Autowired
    public CachingLegalSearcher(ReRankingLegalSearcher delegate, StringRedisTemplate redisTemplate,
            MetricsRecorder metrics, ObjectMapper objectMapper, LegalIndexVersionProvider indexVersions,
            RagProperties properties) {
        this(delegate, redisTemplate, metrics, objectMapper, properties.getCacheTtlMinutes(), indexVersions,
                properties.getCache().getEmbeddingModel(), properties.getCache().getRerankerVersion(),
                properties.getCache().getPipelineVersion());
    }
    ...
    public List<SearchHit> searchScored(RetrievalRequest request, int topK) {
        String contract = request.jurisdiction() + "|" + request.accessScopes().stream().sorted().toList() + "|"
                + request.asOfTime().atZone(ZoneOffset.UTC).toLocalDate();
    ...
    private void delete(String key) {
        try {
            ...
            log.debug("法规缓存删除失败，将继续重新检索 cacheKey={}", key, deleteFailure);
        }
    }
    ...
    private String serialize(Object payload) {
        try {
            ...
            throw new IllegalStateException("法规缓存序列化失败", e);
        }
    }
    ...
    private <T> T deserialize(String json, TypeReference<T> type) {
        try {
            ...
            throw new IllegalStateException("法规缓存反序列化失败", e);
        }
    }
