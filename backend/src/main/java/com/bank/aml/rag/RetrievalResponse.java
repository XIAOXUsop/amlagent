...
/**
 * 可审计检索输出；状态用于驱动 Agent 拒答、重试或人工复核，{@code support} 给出判定原因， {@code traces}
 * 完整保留每阶段分数（SearchHit）供审计与评测使用。
 */
public final class RetrievalResponse {
    ...
    public RetrievalResponse(Status status, String indexVersion, List<RetrievalHit> hits, EvidenceSupport support,
            List<SearchHit> traces) {
        this.status = status;
    ...
    public EvidenceSupport support() {
        return support;
    }

    public List<SearchHit> traces() {
        return traces;
    }
    ...
    public enum Status {
        ...
        SUPPORTED, INSUFFICIENT_EVIDENCE, NO_RELEVANT_EVIDENCE, INDEX_UNAVAILABLE, ACCESS_DENIED
