 ...
 * </ul>
 * <p>
 * 注意：只读 {@link InvestigationSnapshot}（与 Agent/Guardrails 共享同一份冻结事实）， 不直接访问可变数据源与实时 RAG
 * 索引，避免规则降级报告与模型报告之间的数据漂移。
 */
@Component
public class RuleBasedReporter {
    ...
    public RuleBasedReporter() {
        this(new RiskProperties());
    }

    @Autowired
    public RuleBasedReporter(RiskProperties properties) {
        this.transactionPolicy = properties.getTransaction();
    }

    public DueDiligenceReport generate(InvestigationSnapshot snapshot, String alertRule) {
        var customer = snapshot.customer();
        String tx = TransactionTool.format(snapshot.transactions(), customer.id(), transactionPolicy);
        ...
        List<String> evidenceChain = new ArrayList<>(List.of("交易数据源：冻结快照", "工商股权：冻结快照", "黑名单：后端制裁筛查"));
        snapshot.legalEvidence().stream().map(doc -> "法规证据：" + doc.evidenceId()).forEach(evidenceChain::add);

        return new DueDiligenceReport(customer.id(), customer.name(), riskLevel, tx, corp,
                isHit(sanction) ? List.of(sanction) : List.of(), List.of(legal), riskPoints, conclusion(riskLevel),
                evidenceChain, false, findingCodes, actionCodes);
    }
    ...
    public DueDiligenceReport generateSafetyHold(InvestigationSnapshot snapshot, String alertRule,
        ...
        return new DueDiligenceReport(base.customerId(), base.customerName(), risk, base.transactionProfile(),
                base.corporateProfile(), base.sanctions(), base.legalBasis(), points, "模型输出未通过生产契约校验，已生成保守报告并强制转人工复核。",
    ...
    private List<String> findings(InvestigationSnapshot snapshot, String riskLevel) {
        ...
        if (facts.sanctionHit()) {
            if (facts.maxSeverity() == 1)
                findings.add("SANCTION_LEVEL_1_MATCH");
            else if (facts.maxSeverity() >= 2)
                findings.add("DOMESTIC_WATCHLIST_MATCH");
            else
                findings.add("RISK_ASSESSMENT_UNCERTAIN");
        }
        else {
        ...
        if (facts.crossRatio() > 0)
            findings.add("CROSS_BORDER_ACTIVITY");
        if (facts.uboRiskSeverity() >= 2)
            findings.add("UBO_UNVERIFIED");
        else if (facts.uboRiskSeverity() == 1)
            findings.add("UBO_DOCUMENTS_INCOMPLETE");
    ...
    private String assess(String tx, String sanction, List<String> riskPoints) {
        ...
        if (crossRatio > transactionPolicy.getElevatedCrossBorderRatio()
                && nightRatio > transactionPolicy.getElevatedNightRatio()) {
