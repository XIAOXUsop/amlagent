 ...
 *
 * <p>
 * 这里故意不再发起第二次 LLM 调用：前端看到的流式内容与最终评级、发现和动作来自同一份报告， 不会产生双模型调用的额外
 * Token/延迟，也不会出现“流式解释”和最终报告互相矛盾。
 */
@Service
public class FinalReportStreamingService {
    ...
    public FinalReportStreamingService(WorkflowEventService workflowEventService, ExecutorService summaryExecutor) {
        this.workflowEventService = workflowEventService;
        this.summaryExecutor = summaryExecutor;
    }

    /** 提交成功返回 true；提交失败由调用方直接发送终态。 */
    public boolean stream(Long caseId, CaseStatus terminalStatus, DueDiligenceReport report, ExecutionLease lease) {
    ...
    List<String> renderChunks(DueDiligenceReport report) {
        ...
        return List.of("最终风险评级：" + report.riskLevel() + "。\n", "风险发现：" + findings + "。\n", "处置动作：" + actions + "。\n",
                Boolean.TRUE.equals(report.manualReviewRequired()) ? "该工单需要人工复核。" : "该工单可按自动流程继续处理。");
    }
