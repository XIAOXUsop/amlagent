 ...
 * <p>
 * 在 Agent 推理前由 {@code InvestigationSnapshotFactory} 从数据源一次性组装并冻结，
 * 包含交易、股权、制裁的原始领域对象、关联预警、预检索的法规证据与派生风险事实； Agent 工具与 Guardrails 只读本快照，不再二次访问可变业务数据。 报告可通过
 * {@code snapshotId} / {@code sourceDigest} / {@code alertsDigest} / {@code evidenceId}
 * 端到端追溯到数据来源与版本。
 *
 * @param snapshotSchemaVersion 快照结构版本：0=旧 schema（无预警事实），1=包含冻结预警集合。 旧归档缺少新增字段时反序列化为
 * 0，加载与档案导出继续成功。
 */
public record InvestigationSnapshot(String snapshotId, Long caseId, int executionVersion, Instant asOfTime,
        CustomerProfile customer, List<InvestigationAlertSnapshot> alerts, List<TransactionRecord> transactions,
        List<ShareholdingRecord> shareholdings, List<SanctionRecord> sanctionHits, List<LegalDoc> legalEvidence,
        ...
        List<String> legalKeywords, RiskContext riskFacts, String legalIndexVersion,
        /** 冻结业务事实摘要（客户/交易/股权/制裁口径，与历史快照保持一致，不改写其意义）。 */
        String sourceDigest, /** 冻结预警集合摘要：证明模型、法规预检索与档案使用同一份关联预警输入。 */
        String alertsDigest, Integer snapshotSchemaVersion) {
    public InvestigationSnapshot {
        ...
        if (legalEvidenceByTopic == null) {
            ...
            legalEvidenceByTopic
                .forEach((topic, docs) -> frozen.put(topic, docs == null ? List.of() : List.copyOf(docs)));
    ...
    public InvestigationSnapshot(String snapshotId, Long caseId, int executionVersion, Instant asOfTime,
            CustomerProfile customer, List<TransactionRecord> transactions, List<ShareholdingRecord> shareholdings,
            List<SanctionRecord> sanctionHits, List<LegalDoc> legalEvidence,
            Map<String, List<LegalDoc>> legalEvidenceByTopic, List<String> legalKeywords, RiskContext riskFacts,
            String legalIndexVersion, String sourceDigest) {
