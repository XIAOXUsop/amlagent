...
/**
 * 生产数据 Adapter：全部风险事实来自持久化的上游同步表，绝不为缺失数据生成“正常”交易或股权。 空集合表示数据缺失/未同步，由快照数据完整性与 Guardrails
 * 决定是否转人工。
 */
@Component
@Primary
...
public class RelationalCustomerDataPort implements CustomerDataPort, CustomerDataRefresh {
    ...
    private final int sanctionRecallLimit;

    private final Clock clock;

    public RelationalCustomerDataPort(CustomerRepository customers, CustomerTransactionRepository transactions,
            CustomerShareholdingRepository shareholdings, SanctionEntryRepository sanctions, AmlProperties properties,
            Clock clock) {
        this.customers = customers;
        this.transactions = transactions;
        this.shareholdings = shareholdings;
        this.sanctions = sanctions;
        this.sourceVersion = properties.data().sourceVersion();
        this.sanctionRecallLimit = properties.data().sanctionRecallLimit();
        this.clock = clock;
    }
    ...
    public List<CustomerProfile> allCustomers() {
        return customers.findByDeletedFalseOrderByCustomerNoAsc()
            .stream()
            .filter(entity -> "ENABLED".equalsIgnoreCase(entity.getStatus()))
            .map(this::profile)
            .toList();
    ...
    public List<TransactionRecord> transactionsOf(String customerId) {
        return transactions.findByCustomerNoOrderByTransactedAtAsc(customerId)
            .stream()
            .map(entity -> new TransactionRecord(entity.getTransactedAt(), entity.getAmount(), entity.getDirection(),
                    entity.getCounterparty(), region(entity.getCounterpartyRegion()), entity.getChannel(),
                    entity.getPurpose(), entity.getCurrency(), String.valueOf(entity.getId())))
            .toList();
    }

    @Override
    public List<ShareholdingRecord> shareholdingsOf(String customerId) {
        return shareholdings.findByCustomerNoOrderByIdAsc(customerId)
            .stream()
    ...
    public List<SanctionRecord> searchSanctions(String keyword) {
        if (keyword == null || keyword.isBlank())
            return List.of();
        return sanctions.searchEnabled(keyword.trim(), PageRequest.of(0, sanctionRecallLimit))
            .stream()
            .map(entity -> new SanctionRecord(entity.getSubjectName(), entity.getIdentityNumber(), entity.getListName(),
                    entity.getReason(), entity.getSeverity()))
            .toList();
    }

    @Override
    public String sourceSystem() {
        return "BANK_RELATIONAL_SYNC";
    }
    ...
    private CountryRegion region(String value) {
        ...
        catch (IllegalArgumentException unsupportedRegion) {
            return CountryRegion.OTHER;
        }
    }
