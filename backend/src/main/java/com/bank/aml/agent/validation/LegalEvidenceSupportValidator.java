...
import com.bank.aml.rag.LegalActionProposition;
import com.bank.aml.rag.LegalPropositionRegistry;
 ...
 * <p>
 * 基于人工审核的 {@link LegalPropositionRegistry 法律命题表} 做结构化验证：
 * 动作必须命中命题依托法规，且模态（应当/不得/可以）、主体、对象在条款原文中一一对应，不得把“不得冻结”当作支持冻结。
 * </p>
 */
@Component
public class LegalEvidenceSupportValidator {
    ...
    private static final List<String> HIGH_IMPACT_ACTIONS = List.of("FREEZE_ASSETS", "REPORT_TO_AUTHORITY",
            "STOP_FINANCIAL_SERVICE");
    ...
    public List<String> validate(InvestigationSnapshot snapshot, DueDiligenceReport report) {
        if (snapshot == null || report == null || report.actionCodes() == null)
            return List.of();
        Set<String> cited = new LinkedHashSet<>();
        collect(cited, report.legalBasis());
        collect(cited, report.evidenceChain());
        List<LegalDoc> citedDocs = snapshot.legalEvidence()
            .stream()
            .filter(doc -> cited.contains(doc.evidenceId()))
            .toList();
        List<String> violations = new ArrayList<>();
        for (String actionCode : HIGH_IMPACT_ACTIONS) {
            if (!report.actionCodes().contains(actionCode))
                continue;
            ...
            boolean supported = propositions.stream()
                .anyMatch(proposition -> citedDocs.stream().anyMatch(doc -> supportedBy(doc, proposition)));
            if (!supported) {
    ...
    private boolean supportedBy(LegalDoc doc, LegalActionProposition proposition) {
        ...
        if ("FREEZE_ASSETS".equals(proposition.actionCode()) && (text.contains("不得冻结") || text.contains("禁止冻结"))) {
            return false;
        }
