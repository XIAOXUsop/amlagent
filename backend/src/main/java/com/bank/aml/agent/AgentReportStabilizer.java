 ...
 *
 * <p>
 * 法规证据在调用模型前已经由后端检索并冻结。模型偶尔会在结构化输出中遗漏或只在一侧 写入 evidenceId；这里仅把冻结快照中真实存在的证据引用同步到
 * legalBasis/evidenceChain， 不补风险代码、不修改评级，也不接受模型自造的证据 ID。后续仍由 {@code AgentOutputValidator}
 * 校验证据归属及高影响处置是否得到条文内容支持。
 * </p>
 */
public final class AgentReportStabilizer {
    ...
    public static DueDiligenceReport attachFrozenLegalEvidence(InvestigationSnapshot snapshot,
     ...
     */
    public static DueDiligenceReport attachFrozenLegalEvidence(InvestigationSnapshot snapshot,
            DueDiligenceReport report, Collection<String> returnedEvidenceIds) {
        ...
        return new DueDiligenceReport(report.customerId(), report.customerName(), report.riskLevel(),
                report.transactionProfile(), report.corporateProfile(), report.sanctions(), List.copyOf(legalBasis),
                report.riskPoints(), report.conclusion(), List.copyOf(evidenceChain), report.manualReviewRequired(),
    ...
    private static String formatBasis(LegalDoc doc) {
        ...
        if (doc.title() != null && !doc.title().isBlank())
            parts.add(doc.title());
        if (doc.articleNumber() != null && !doc.articleNumber().isBlank())
            parts.add(doc.articleNumber());
    ...
    private static Set<String> extractAllowed(List<String> values, Set<String> allowedIds) {
        ...
        for (String value : values) {
            ...
            while (matcher.find()) {
                if (allowedIds.contains(matcher.group()))
