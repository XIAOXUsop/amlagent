 ...
 * <p>
 * 快照工具只读 {@link InvestigationSnapshot}（含预检索法规证据），不再访问 {@code CustomerDataPort} 或可变 RAG
 * 索引，从而保证 Agent 推理与 Guardrails 校验共享同一份业务事实（Snapshot First）。
 */
@Component
public class DueDiligenceAgentFactory {
    ...
    public DueDiligenceAgentFactory(@Qualifier("mainAgentChatModel") ChatModel chatModel, RiskProperties properties,
    ...
    public AgentWithTools createWithTraces(InvestigationSnapshot snapshot) {
        SnapshotToolSuite tools = new SnapshotToolSuite(snapshot, transactionPolicy);
        DueDiligenceAgent agent = build(chatModel, tools, maxToolRoundTrips);
        return new AgentWithTools(agent, tools);
    }

    /** 生产与离线评测共享的 Agent 构建策略，防止工具并发和轮次上限发生配置漂移。 */
    public static DueDiligenceAgent build(ChatModel chatModel, Object tools, int maxToolRoundTrips) {
