 ...
 * 生产 Agent 输出契约校验器。
 * <p>
 * 所有检查均为确定性检查：结构、闭集词表、人工复核一致性、法规证据归属和关键事实前置条件。 该组件不自动吞掉或改写模型错误，调用方必须显式决定重试、降级或转人工。
 */
@Component
public class AgentOutputValidator {

    private static final Pattern LEGAL_EVIDENCE_ID = Pattern
        .compile("(?i)\\b(?:[A-Z0-9]+-)*LEGAL-[A-Z0-9][A-Z0-9_-]*\\b");
    ...
    public AgentOutputValidator() {
        this(new LegalEvidenceSupportValidator(), new PromptInjectionGuard(), new AgentOutputProperties());
    }

    @Autowired
    public AgentOutputValidator(LegalEvidenceSupportValidator evidenceSupportValidator,
            PromptInjectionGuard injectionGuard, AgentOutputProperties properties) {
        this.evidenceSupportValidator = evidenceSupportValidator;
        this.injectionGuard = injectionGuard;
        this.maxTextCharacters = properties.getMaxTextCharacters();
        this.maxListItems = properties.getMaxListItems();
    }
    ...
    public ValidationResult validate(InvestigationSnapshot snapshot, DueDiligenceReport report) {
        ...
        requireList(report.findingCodes(), "FINDING_CODES", true, AgentReportVocabulary.FINDING_CODES, violations);
        requireList(report.actionCodes(), "ACTION_CODES", true, AgentReportVocabulary.ACTION_CODES, violations);
    ...
    private void validateIdentityLeakage(InvestigationSnapshot snapshot, DueDiligenceReport report,
        ...
        boolean leaked = containsIdentifier(content, customerId)
                || (!customerName.isBlank() && content.contains(customerName)) || containsIdentifier(content, idCard);
        if (leaked) {
    ...
    private void addAll(List<String> values, List<String> additions) {
        if (additions != null)
            additions.stream().filter(Objects::nonNull).forEach(values::add);
    }
    ...
    private void validateLegalEvidence(InvestigationSnapshot snapshot, DueDiligenceReport report,
            List<String> violations) {
        Set<String> allowed = snapshot.legalEvidence()
            .stream()
    ...
    private void validateFactPrerequisites(RiskContext facts, DueDiligenceReport report, List<String> violations) {
        ...
        unsupported(findings.contains("NO_SANCTION_HIT") && facts.sanctionHit(), "NO_SANCTION_HIT_UNSUPPORTED",
                violations);
        unsupported(findings.contains("SANCTION_LEVEL_1_MATCH") && facts.maxSeverity() != 1,
                "SANCTION_LEVEL_1_MATCH_UNSUPPORTED", violations);
        unsupported(findings.contains("DOMESTIC_WATCHLIST_MATCH") && (!facts.sanctionHit() || facts.maxSeverity() < 2),
        ...
        unsupported(findings.contains("UBO_UNVERIFIED") && facts.uboRiskSeverity() < 2, "UBO_UNVERIFIED_UNSUPPORTED",
                violations);
        ...
        unsupported(actions.contains("FREEZE_ASSETS") && facts.maxSeverity() != 1, "FREEZE_ASSETS_UNSUPPORTED",
                violations);
        boolean suspiciousFacts = facts.maxSeverity() == 1 || facts.transactionPatternSeverity() >= 2
                || findings.contains("RISK_ASSESSMENT_UNCERTAIN") || findings.contains("TRANSACTION_PURPOSE_UNVERIFIED")
                || findings.contains("SOURCE_OF_FUNDS_UNVERIFIED");
        unsupported(actions.contains("REPORT_TO_AUTHORITY") && !suspiciousFacts, "REPORT_TO_AUTHORITY_UNSUPPORTED",
                violations);
    }
    ...
    private void requireText(String value, String code, List<String> violations) {
        ...
        else if (value.length() > maxTextCharacters) {
            violations.add(code + "_TOO_LONG");
        }
    }

    private void requireList(List<String> values, String code, boolean nonEmpty, Set<String> vocabulary,
            List<String> violations) {
        if (values == null) {
        ...
        if (values.stream().filter(value -> value != null).anyMatch(value -> value.length() > maxTextCharacters)) {
            violations.add(code + "_ITEM_TOO_LONG");
        }
        if (vocabulary != null
                && values.stream().filter(Objects::nonNull).anyMatch(value -> !vocabulary.contains(value))) {
