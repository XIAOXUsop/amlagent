 ...
 *   <li>{@link #fromAlertRuleText}：版本 0 / 不绑定数据库案件的评测夹具的显式兼容适配器，
 *       只允许存量兼容路径调用，禁止掩盖版本 1 的数据缺失。</li>
 * </ul>
...
public class AlertSnapshotAssembler {
    ...
    public AlertSnapshotAssembler(ObjectMapper objectMapper,
                                  @Value("${aml.agent.max-linked-alerts:8}") int maxLinkedAlerts) {
    ...
    public List<InvestigationAlertSnapshot> fromLinkedAlerts(Long caseId, List<AmlAlert> alerts) {
        ...
        for (AmlAlert alert : alerts) {
            if (alert == null || alert.getStatus() != AlertStatus.LINKED
                    || !java.util.Objects.equals(alert.getCaseId(), caseId)) {
                continue;
            }
            frozen.add(new InvestigationAlertSnapshot(alert.getId(), alert.getExternalAlertId(),
                    alert.getRuleCode(), alert.getScenarioCode(), alert.getHitReason(),
                    alert.getOccurredAt(), alert.getRevision()));
        }
        frozen.sort(Comparator.comparing(InvestigationAlertSnapshot::occurredAt,
                Comparator.nullsLast(Comparator.naturalOrder()))
                .thenComparing(InvestigationAlertSnapshot::alertId,
                        Comparator.nullsLast(Comparator.naturalOrder())));
        if (frozen.size() > maxLinkedAlerts) {
            throw new NonRetryableWorkflowException("案件关联预警数量 " + frozen.size()
                    + " 超出单次尽调容量上限 " + maxLinkedAlerts + "，请先归并到更少案件或拆分后分别调查");
        }
        return List.copyOf(frozen);
    }

    /**
     * 兼容路径：版本 0 / 评测夹具只有预警文本时的显式适配。
     * 生成一条标记为 LEGACY_ALERT_RULE 的伪预警，仅用于保持旧快照/旧评测语义，不代表真实预警记录。
     */
    ...
    /**
     * 预警输入摘要：对规范化序列化后的预警字段做 SHA-256。
     * 包含命中原因与版本，因此修改原因或归并状态变化都能通过摘要识别；不使用有歧义的字符串简单拼接。
     */
