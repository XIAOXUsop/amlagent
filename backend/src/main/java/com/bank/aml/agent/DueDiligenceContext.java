 ...
 * </ul>
 * 关联预警逐条进入模型输入（而非截断至 255 字的案件摘要），并保留编号对应关系，
 * 允许工具和报告追溯到具体预警；预警文本全部按不可信业务数据处理。
 */
public record DueDiligenceContext(
    ...
    public String toPrompt() {
        ...
        for (int i = 0; i < linkedAlerts.size(); i++) {
            ...
            alertsSection.append(String.format(
                    "- 预警 %d［%s］规则=%s 场景=%s 发生时间=%s 命中原因：%s%n",
                    i + 1,
        ...
        return """
                ...
                String.join("、", legalSearchTopics), alertRule, linkedAlerts.size(),
                alertsSection.isEmpty() ? "- （无关联预警）\n" : alertsSection, caseDescription);
    }
}
