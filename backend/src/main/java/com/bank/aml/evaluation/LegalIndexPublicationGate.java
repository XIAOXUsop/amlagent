...
import com.bank.aml.rag.ingestion.RagPublicationEvaluator;
 ...
 * <p>
 * 检查项：候选评测 Recall@5 / nDCG@5 / 拒答率 / 拒答准确率、冷 P95 延迟、与 active 的 Recall 回退幅度。
 * 其余隔离类门禁（失效法律隔离、ACL 隔离、投毒文档隔离、evidenceId 重复、索引完整性） 由分块 fail-closed、安全扫描与
 * {@code candidateCount==segmentCount} 在构建路径内保证。
 * </p>
 */
@Component
public class LegalIndexPublicationGate implements RagPublicationEvaluator {
    ...
    public LegalIndexPublicationGate(RagEvaluator evaluator, LegalIndexVersionProvider versions, double minRecallAt5,
            double minNdcgAt5, double maxRecallDropVsActivePp, double minAbstentionAccuracy,
            double minNoAnswerRefusalRate, double maxColdP95Ms) {
        this(evaluator, versions, null, minRecallAt5, minNdcgAt5, maxRecallDropVsActivePp, minAbstentionAccuracy,
                minNoAnswerRefusalRate, maxColdP95Ms);
    }

    @Autowired
    public LegalIndexPublicationGate(RagEvaluator evaluator, LegalIndexVersionProvider versions,
            MetricsRecorder metrics, RagProperties properties) {
        this(evaluator, versions, metrics, properties.getPublicationGate().getMinRecallAt5(),
                properties.getPublicationGate().getMinNdcgAt5(),
                properties.getPublicationGate().getMaxRecallDropVsActivePp(),
                properties.getPublicationGate().getMinAbstentionAccuracy(),
                properties.getPublicationGate().getMinNoAnswerRefusalRate(),
                properties.getPublicationGate().getMaxColdP95Ms());
    }

    private LegalIndexPublicationGate(RagEvaluator evaluator, LegalIndexVersionProvider versions,
            MetricsRecorder metrics, double minRecallAt5, double minNdcgAt5, double maxRecallDropVsActivePp,
            double minAbstentionAccuracy, double minNoAnswerRefusalRate, double maxColdP95Ms) {
    ...
    public GateResult evaluate(String candidateVersion, int segmentCount) {
        ...
        if (!active.isBlank() && !active.equals(candidateVersion)) {
            ...
            if (drop > maxRecallDropVsActivePp) {
                failures.add("recallDropVsActivePp=" + drop + " > " + maxRecallDropVsActivePp + " (active="
                        + activeReport.recallAt5() + ", candidate=" + report.recallAt5() + ")");
            }
            double overlap = candidateActiveOverlap(activeReport, report);
            if (metrics != null)
                metrics.ragCandidateActiveOverlap(overlap);
        }
        ObjectNode gates = MAPPER.createObjectNode();
        for (String failure : failures)
            gates.put("failed_" + (gates.size() + 1), failure);
        ...
        if (!failures.isEmpty()) {
            report.details()
                ...
                .forEach(detail -> log.warn(
                        ...
                        detail.id(), detail.category(), detail.answerable(), detail.rank(), detail.retrievalStatus(),
                        detail.support(), detail.returnedEvidenceIds(), detail.relevanceScores()));
        }
    ...
    private double candidateActiveOverlap(RagEvaluator.RagEvalReport active, RagEvaluator.RagEvalReport candidate) {
        ...
        if (n == 0)
            return 0;
        for (int i = 0; i < n; i++) {
            Set<String> a = new HashSet<>(active.details().get(i).returnedEvidenceIds());
            Set<String> c = new HashSet<>(candidate.details().get(i).returnedEvidenceIds());
            if (c.isEmpty())
                continue;
            a.retainAll(c);
            intersect += a.size();
