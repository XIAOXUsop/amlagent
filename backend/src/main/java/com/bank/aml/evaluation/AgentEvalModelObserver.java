 ...
 *
 * <p>
 * The wrapper delegates model capabilities unchanged so LangChain4j can still select its
 * structured-output strategy. It records only run diagnostics; it never substitutes a
 * response.
 * </p>
 */
final class AgentEvalModelObserver implements ChatModel {
    ...
    Snapshot snapshot() {
        ...
        return new Snapshot(requestCount.get(), inputTokens.get(), outputTokens.get(), totalTokens.get(),
                lastModelName.get(), lastAssistantText.get(),
    ...
    private void record(ChatResponse response) {
        ...
        if (message != null && message.hasToolExecutionRequests()) {
            message.toolExecutionRequests()
                .forEach(request -> requestedTools.add(new RequestedToolCall(request.name())));
        }
    ...
    record Snapshot(int requestCount, int inputTokens, int outputTokens, int totalTokens, String modelName,
            String lastAssistantText, String error, List<RequestedToolCall> requestedTools) {
    }
