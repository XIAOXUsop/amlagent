...
final class DeepSeekRequestSanitizer {
    ...
    private static final Set<String> UNSUPPORTED_CUSTOM_PARAMETERS = Set.of("prompt_cache_retention",
    ...
    static ChatRequest sanitize(ChatRequest request) {
        ...
        return ChatRequest.builder().messages(request.messages()).parameters(sanitized).build();
    }
    ...
    static ChatRequestParameters sanitize(ChatRequestParameters parameters) {
        ...
        if (custom == null || custom.isEmpty()
                || custom.keySet().stream().noneMatch(DeepSeekRequestSanitizer::isUnsupported)) {
            return parameters;
        }
        ...
        return OpenAiChatRequestParameters.builder().overrideWith(openAiParameters).customParameters(sanitized).build();
    }
