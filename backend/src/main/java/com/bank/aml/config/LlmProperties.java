...
/**
 * 多 LLM 提供商配置。通过 {@code aml.llm.active-provider} 切换默认提供商， {@code aml.llm.providers}
 * 定义各提供商连接参数（OpenAI 兼容 / Anthropic / Mock）。
 */
...
public class LlmProperties {
    ...
    @NotEmpty
    private Map<String, @Valid LlmProviderProperties> providers = new HashMap<>();
    ...
    public LlmProviderProperties active() {
        ...
        if (p == null) {
            throw new IllegalStateException(
                    "未找到 LLM 提供商配置: " + activeProvider + "，请在 application.yml 的 aml.llm.providers 中定义");
        }
        return p;
    }
