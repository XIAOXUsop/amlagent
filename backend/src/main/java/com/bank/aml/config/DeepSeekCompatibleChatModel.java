 ...
 *
 * <p>
 * 部分上游 OpenAI 客户端会在单次请求中附加 extended prompt cache 参数， 但 DeepSeek
 * 的上下文缓存由服务端自动启用，不支持这些字段。该包装器在最终出站前 删除不兼容字段，避免请求以
 * {@code prompt_cache_retention is not supported on this model} 失败；其他参数（包括 DeepSeek 的
 * thinking 配置）保持不变。
 * </p>
 */
