 ...
 * <p>
 * 请求结束前清理，避免 MDC 泄漏到下一条请求。配合日志格式中的 %X{traceId}， 可在日志中按一次请求聚合关联排查。
 * <p>
 * 同一 traceId 会写入 {@link HttpServletRequest} 属性与响应头 {@code X-Request-Id}， 使
 * {@code GlobalExceptionHandler} 返回的 traceId 与日志 MDC、下游透传三方保持一致， 便于端到端关联。客户端透传的
 * X-Request-Id 需通过字符/长度白名单校验，防止日志注入。
 */
@Component
