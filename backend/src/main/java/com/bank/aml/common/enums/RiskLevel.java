...
/**
 * 客户风险等级统一枚举：作为 Guardrails / 规则引擎 / 评测三方共享的单一事实来源， 消除散落各处的"低风险/中风险/高风险"魔法字符串与等级分值映射。
 * <p>
 * 存储与 API 契约仍使用中文文案（低风险/中风险/高风险），本枚举只负责文案↔分值↔序数换算。
 */
public enum RiskLevel {

    LOW("低风险", 1), MEDIUM("中风险", 2), HIGH("高风险", 3);
    ...
    public static RiskLevel fromLabel(String label) {
        return Optional.ofNullable(label)
            .map(String::trim)
            .flatMap(l -> Arrays.stream(values()).filter(v -> v.label.equals(l)).findFirst())
            .orElse(LOW);
    }
