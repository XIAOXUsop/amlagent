 ...
 * <p>统一映射为 HTTP 409 与稳定错误码 {@code INVESTIGATION_REVISION_CONFLICT}；
 * 客户端必须刷新调查事实后由用户明确重新确认，禁止自动重放旧结论。
 * <p>携带有界的冲突对象信息（类型/标识/当前版本），让页面可以区分
 * “覆盖被他人确认”（type=COVERAGE）与“假设被改判”（type=HYPOTHESIS），
...
public class InvestigationRevisionConflictException extends RuntimeException {
    ...
    public InvestigationRevisionConflictException(String conflictType, Long conflictId,
                                                  Integer currentVersion, String message) {
        super(message);
