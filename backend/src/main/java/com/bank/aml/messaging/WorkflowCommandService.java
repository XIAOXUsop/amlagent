...
/**
 * 工单入队命令服务：所有"状态迁移 + 入队"在同一事务内完成，入队统一写入 Outbox， 由 {@link OutboxPublisher} 异步投递到 Redis
 * Streams，避免状态变更与消息投递不一致。
 * <p>
 * {@code RetryScheduler} / {@code PendingClaimer} / {@code QueueController} 均通过本服务入队，
...
public class WorkflowCommandService {
    ...
    public boolean reclaimExpiredCase(Long caseId, int executionVersion, String worker,
        ...
        if (caseRepository.reclaimStuckCase(caseId, CaseStatus.PENDING, CaseStatus.RUNNING, executionVersion, worker,
    ...
    public boolean failReclaimExhausted(Long caseId, int executionVersion, String worker,
            LocalDateTime heartbeatThreshold) {
        return caseRepository.failReclaimExhausted(caseId, CaseStatus.FAILED, CaseStatus.RUNNING, executionVersion,
                worker, heartbeatThreshold) == 1;
    }
    ...
    public boolean markDeadLetter(Long caseId, String worker, int executionVersion, int retry, String message) {
        if (caseRepository.failCase(caseId, CaseStatus.FAILED, retry, "RETRY_EXHAUSTED", message, worker,
                executionVersion) == 1) {
