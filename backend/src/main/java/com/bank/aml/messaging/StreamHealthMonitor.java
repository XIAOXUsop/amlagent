 ...
 * <p>
 * 周期驱动 {@link WorkflowConsumer#probeLag()}，把"瞬时尖峰"与"持续停摆"区分开， 对检测到的消费者停滞 / 连接中断做分级响应：
 * <ol>
 * <li><b>连接中断</b>：探测失败一次即触发消费者容器重建（自助恢复）；</li>
 * <li><b>持续积压</b>：lag 连续超过告警阈值且持续多个周期后记录告警日志； 若继续走高超过恢复阈值，则强制重建消费者容器以恢复消费。</li>
 * </ol>
 * <p>
 * 阈值来自 {@code aml.queue} 配置，见 {@link QueueProperties}。
 */
@Component
public class StreamHealthMonitor {
    ...
    @Scheduled(fixedDelayString = "${aml.queue.health-probe-seconds}000",
            initialDelayString = "${aml.queue.health-initial-delay-seconds}000")
    public void monitor() {
        ...
        // 持续达到恢复阈值：强制重建（自助恢复）
        if (lagRecover > 0 && unhealthyCycles >= Math.max(props.getMinimumLagRecoveryCycles(),
                props.getConsecutiveErrorAlertThreshold()) && lag >= lagRecover) {
            log.error("ALERT Redis Streams 消费者停滞，lag={} 持续 {} 个周期超过恢复阈值 {}，强制重建容器", lag, unhealthyCycles, lagRecover);
        ...
        if (lagAlert > 0 && !alerted && unhealthyCycles >= Math.max(1, props.getConsecutiveErrorAlertThreshold())
                && lag >= lagAlert) {
            log.error("ALERT Redis Streams 消费者疑似停滞，消息积压 lag={} 持续 {} 个周期（告警阈值 {}，恢复阈值 {}）", lag, unhealthyCycles,
                    lagAlert, lagRecover);
            alerted = true;
        }
    }
