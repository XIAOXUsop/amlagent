...
public class SuspiciousTransactionReportController {
    ...
    @GetMapping("/{caseId}")
    public SuspiciousTransactionReportView get(@PathVariable Long caseId) {
        return service.get(caseId);
    }

    /** 登记外部可疑交易报告系统已受理；本接口本身不冒充外部报送连接器。 */
    @PostMapping("/{caseId}/submit")
    public SuspiciousTransactionReportView submit(@PathVariable Long caseId, @Valid @RequestBody SubmitRequest request,
            Authentication authentication) {
        SuspiciousTransactionReportView result = service.markSubmitted(caseId, request.expectedRevision(),
                request.externalReference(), authentication.getName());
        return result;
    }

    @PostMapping("/{caseId}/return")
    public SuspiciousTransactionReportView returnForCorrection(@PathVariable Long caseId,
            @Valid @RequestBody ReturnRequest request, Authentication authentication) {
        SuspiciousTransactionReportView result = service.returnForCorrection(caseId, request.expectedRevision(),
    ...
    public record ReturnRequest(@PositiveOrZero int expectedRevision,
