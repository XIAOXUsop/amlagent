...
public class AlertController {
    ...
    public AlertController(CaseIntakeService intake, AlertScopeService alertScopeService) {
        this.intake = intake;
    ...
    public AlertView create(@Valid @RequestBody CreateAlertRequest request, Authentication authentication) {
        return intake.createAlert(request.externalAlertId(), request.customerId(), request.ruleCode(),
                request.scenarioCode(), request.hitReason(), toUtcLocalDateTime(request.occurredAt()),
    ...
    /** 在启用解释政策前，由管理员把上游命中范围按当前来源版本冻结到服务端。 */
    @PostMapping("/{alertId}/scope/freeze")
    @PreAuthorize("hasRole('ADMIN')")
    public AlertScopeService.ScopeSnapshot freezeScope(@PathVariable Long alertId,
            @Valid @RequestBody ScopeFreezeRequest request, Authentication authentication) {
        return alertScopeService.freezeScope(alertId, request.triggerTransactionIds(),
                request.auxiliaryTransactionIds(), request.sourceVersion(), authentication.getName());
    }

    @PostMapping("/{alertId}/link")
    public AlertView link(@PathVariable Long alertId, @Valid @RequestBody LinkRequest request,
            Authentication authentication) {
        return intake.linkToCase(alertId, request.caseId(), request.expectedRevision(), request.reason(),
                authentication.getName());
    }
    ...
    public record CreateAlertRequest(@NotBlank @Size(min = 3, max = 64) String externalAlertId,
            @NotBlank @Size(max = 32) String customerId, @NotBlank @Size(min = 3, max = 64) String ruleCode,
            @Size(max = 64) String scenarioCode, @NotBlank @Size(min = 10, max = 500) String hitReason,
            Instant occurredAt) {
    }

    public record CaseActionRequest(@PositiveOrZero int expectedRevision, Boolean autoProcess,
            Boolean enableExplanationPolicy) {
    }

    public record LinkRequest(@NotNull @PositiveOrZero Long caseId, @PositiveOrZero int expectedRevision,
    ...
    public record ScopeFreezeRequest(
            @NotNull @Size(max = 500) List<@NotBlank @Size(max = 128) String> triggerTransactionIds,
            @Size(max = 500) List<@NotBlank @Size(max = 128) String> auxiliaryTransactionIds,
