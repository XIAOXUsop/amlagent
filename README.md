...
### mysql-connector-j：2 条已升级到 26.7.0（**待扫描确认**）
...
> **这一条暂时**没有**写「已清除」。** 按本节上面那条判据——**「清掉了」是靠下一次扫描的
> 输出里那些包不再出现，不是靠推断**。dependency-check 跑一次要一个多小时，
> 等 CI 那次结果出来再改措辞。在那之前，上面的「15 条」是个**待确认**的数。
