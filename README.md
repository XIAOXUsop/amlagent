...
| 后端集成回归（`-Pintegration-test`，需 MySQL/PGVector/Redis） | 43/43 | 25 | **18** | 0 | 2026-09-18 05:05 |
...
> **集成回归当前是红的，18 项失败，这一条如实写在这里。** 它们不是"跑不起来"，
> 而是断言与现状不符或彼此相互污染，逐类看是：
> 退款台账的归属校验与并发串行化（6）、解释工作区的来源版本不一致（3）、
> 工作流等待终态超时与状态机断言（5）、PGVector 表名白名单与迁移基线（2）、
> RAG 评测阈值（1）、以及一项已修的 CSRF 用例（原先断言 200，而建单接口按语义返回 201）。
>
> 这些测试**各自建自己的库**（`aml_refund_ledger_test`、`aml_migration_test` 等），
> 所以不是"共用开发库导致的脏数据"；`application-test.yml` 里的默认数据源只是给
> 没有自建数据源的用例兜底。

### 这批失败的共同来源

退款、解释工作区、PGVector 三个集群的失败信息，`git log -S` 追下去都指向同一个提交：

```
27b1955  style: normalize repository code and engineering conventions
```

它的标题写着 style，实际改了 **703 个文件、+46009 / −28753 行**，并且**改变了行为**：

- 给 `RefundLedgerService.register` 新增了「分配必须指向案件客户的权威交易来源」校验——
  这条校验本身是对的（不能让调用方凭空声称一笔原付款），但测试里的夹具仍是旧的 `T-1001`；
- 新增了「预警范围来源版本必须与当前权威来源版本一致」校验；
- 新增了 PGVector 表名的白名单。

三处都是**代码对、测试旧**，不是测试发现了真问题。同一提交还顺带弄坏了另外三件事
（后端起不来、前端白屏、客户列表被契约整体丢弃），那些已经在本次修复中处理。

> 写在这里是因为它决定了怎么修剩下的部分：这些不是"18 个互不相干的 bug"，
> 而是一次未同步测试的行为变更的余波，每个集群都有明确的入手点。
