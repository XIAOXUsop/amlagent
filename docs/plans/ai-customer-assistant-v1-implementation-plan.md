...
> ⚠️ **本节是 2026-08-23 写这份计划时的基线，不是现在的状态。**
> 2026-09-19 对照仓库核过，下面几条已经对不上了：
>
> | 本节写的 | 现在 |
> |---|---|
> | Spring Boot 3.5.13 | **3.5.16** |
> | LangChain4j 1.18.1 / 1.18.1-beta28 | **1.20.0 / 1.20.0-beta30** |
> | 最新迁移 `V15` | **`V36`**（`V36__refund_ledger_integrity_constraints.sql`） |
>
> 保留原值而不是改成现在的数——**这份计划记录的是"当时基于什么做判断"**，
> 把基线改新会让它描述的推理链和结论对不上。要看当前基线，去仓库 README 与 `backend/pom.xml`。
