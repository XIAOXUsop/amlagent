...
> ⚠️ **上面那个 2423 是"动手前"的数，别当成当前值**——本文记录的这次提取之后是 **2272 行**
> （见「结果」）。2026-09-19 复核时我照着背景这句去找 2423 行的类，量出来 2272，一度以为
> 这个数字是错的；实际是两处指的时间点不同。写在这里免得下一个人再走一遍。
>
> **三次提取的完整轨迹**（`git show <commit>:<file> | wc -l`，可直接复算）：
>
> | 提交 | 行数 | 那次做了什么 |
> |---|---:|---|
> | `27b1955` | 2535 | 提取前 |
> | `45095fc` | 2493 | 抽出草稿 JSON 读写工具类 |
> | `9c06457` | 2423 | 抽出领域事实字典与权威交易事实 |
> | `bd17bd7` | **2272** | 抽出 `GroupPaymentClaimValidator`（本文记录的那一步） |
>
> **三次提取一共只减掉 263 行（约 10%）**——这个斜率本身就是 A5 决策的依据之一：
> 按职责提取的收益递减得很明显，而剩下的耦合穿过的是共享助手，不是方法簇。
